...
sealed class TgSyncState {
    ...
    data class Running(val progress: Int, val total: Int) : TgSyncState()
    data class Completed(val itemsProcessed: Int) : TgSyncState()
    data class Failed(val error: String) : TgSyncState()
}
...
sealed class TgActivityEvent {
    data class NewMessage(val chatId: Long, val messageId: Long) : TgActivityEvent()
    data class NewDownload(val fileId: Int, val fileName: String) : TgActivityEvent()
    data class DownloadComplete(val fileId: Int, val fileName: String) : TgActivityEvent()
    data class ParseComplete(val chatId: Long, val itemsFound: Int) : TgActivityEvent()
}
...
class T_TelegramServiceClient private constructor(
    ...
    suspend fun ensureStarted(context: Context, settings: SettingsStore) {
        ...
        try {
            ...
            session = T_TelegramSession(
                client = client!!,
                config = config!!,
                scope = serviceScope
            )
            
            browser = T_ChatBrowser(
                session = session!!
            )
            
            downloader = T_TelegramFileDownloader(
                context = context,
                session = session!!
            )
     ...
     */
    suspend fun login(phone: String? = null, code: String? = null, password: String? = null) {
     ...
     */
    suspend fun listChats(context: Context, limit: Int = 100): List<Chat> {
    ...
    fun downloader(): T_TelegramFileDownloader {
        return downloader ?: throw IllegalStateException("Service not started")
    }
     ...
     */
    fun browser(): T_ChatBrowser {
        return browser ?: throw IllegalStateException("Service not started")
    }

...
object FishLogging {
    ...
    suspend fun logSeriesFocus(ctx: Context, media: MediaItem) {
        val sid = media.streamId ?: return
        val obxTitle = withContext(Dispatchers.IO) {
            runCatching {
                val store = com.chris.m3usuite.data.obx.ObxStore.get(ctx)
        ...
        com.chris.m3usuite.core.debug.GlobalDebug.logTileFocus("series", sid.toString(), media.name, obxTitle)
        com.chris.m3usuite.core.debug.GlobalDebug.logTree("row:series", "tile:$sid")
    }

    suspend fun logLiveFocus(ctx: Context, media: MediaItem) {
        val sid = media.streamId ?: return
        val obxTitle = withContext(Dispatchers.IO) {
            runCatching {
                val store = com.chris.m3usuite.data.obx.ObxStore.get(ctx)
        ...
        com.chris.m3usuite.core.debug.GlobalDebug.logTileFocus("live", sid.toString(), media.name, obxTitle)
        com.chris.m3usuite.core.debug.GlobalDebug.logTree("row:live", "tile:$sid")
    }

    suspend fun logTelegramFocus(ctx: Context, media: MediaItem) {
        val key = media.tgMessageId ?: media.id
        com.chris.m3usuite.core.debug.GlobalDebug.logTileFocus("telegram", key.toString(), media.name, null)
        com.chris.m3usuite.core.debug.GlobalDebug.logTree("row:telegram", "tile:$key")
    }
}

...
class ProfileObxRepository(private val context: Context) {
    private val store get() = ObxStore.get(context)

    suspend fun all(): List<ObxProfile> = withContext(Dispatchers.IO) {
        store.boxFor(ObxProfile::class.java).all
    }

    suspend fun byId(id: Long): ObxProfile? = withContext(Dispatchers.IO) {
        store.boxFor(ObxProfile::class.java).get(id)
    }

    suspend fun insert(p: ObxProfile): Long = withContext(Dispatchers.IO) {
        store.boxFor(ObxProfile::class.java).put(p)
    }

    suspend fun update(p: ObxProfile) = withContext(Dispatchers.IO) {
        store.boxFor(ObxProfile::class.java).put(p)
    }

    suspend fun delete(p: ObxProfile) = withContext(Dispatchers.IO) {

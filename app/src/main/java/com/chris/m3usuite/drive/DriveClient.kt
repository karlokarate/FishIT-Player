...
object DriveDefaults {
    // Fixed to user's shared folder
    const val DEFAULT_FOLDER_ID = "18UrvlyDSHdmmf3jBm5A0jUGzzDo38-4E"
}
...
object DriveClient {
    ...
    fun signIn(activity: Activity, onResult: (Boolean) -> Unit) {
        ...
        client.silentSignIn().addOnCompleteListener {
            if (it.isSuccessful) onResult(true) else {
                activity.startActivityForResult(client.signInIntent, 9910)
                onResult(true)
            }
        }
    }
    ...
    private fun service(ctx: Context): Drive {
        ...
        val credential = GoogleAccountCredential.usingOAuth2(ctx, setOf(DriveScopes.DRIVE_FILE))
        credential.selectedAccount = account.account
        return Drive.Builder(AndroidHttp.newCompatibleTransport(), GsonFactory.getDefaultInstance(), credential)
            .setApplicationName("m3uSuite")
            .build()
    }

...
// Separate public handle to the same "settings" datastore used by SettingsStore.
...
object SettingsSnapshot {
    suspend fun dump(context: Context): Map<String, String> = withContext(Dispatchers.IO) {
        ...
        // Serialize all values to string to ensure portability across types.
    ...
    suspend fun restore(context: Context, values: Map<String, String>, replace: Boolean) = withContext(Dispatchers.IO) {
        context.settingsBackupDataStore.edit { prefs ->
            ...
            for ((name, s) in values) {
                // Try to infer numeric/boolean types; fallback to string.
                when {
                    s.equals("true", ignoreCase = true) || s.equals("false", ignoreCase = true) -> {
                        prefs[booleanPreferencesKey(name)] = s.equals("true", ignoreCase = true)
                    }
                    s.toLongOrNull() != null -> {
                        val lv = s.toLong()
                        if (lv in Int.MIN_VALUE..Int.MAX_VALUE) prefs[intPreferencesKey(name)] = lv.toInt()
                        else prefs[longPreferencesKey(name)] = lv
                    }
                    s.toFloatOrNull() != null -> {
                        prefs[floatPreferencesKey(name)] = s.toFloat()
                    }
                    else -> {
                        prefs[stringPreferencesKey(name)] = s
                    }
                }
            }
        }

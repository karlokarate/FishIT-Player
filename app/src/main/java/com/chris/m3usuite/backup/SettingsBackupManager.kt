...
class SettingsBackupManager(
    ...
    suspend fun exportAll(
        progress: suspend (Int, String) -> Unit,
        ...
        withContext(Dispatchers.IO) {
            ...
            val filename =
                "m3usuite-settings-v1-" + DateTimeFormatter.ofPattern("yyyyMMdd-HHmm").format(java.time.LocalDateTime.now()) + ".msbx"
            progress(100, "Fertig")
            filename to bytes
        }
            ...
            for (p in payload.profiles) {
                ...
                val avatarPath =
                    p.avatarFile?.let { path ->
                        ...
                        val out = File(outDir, "avatar.$ext")
                        out.writeBytes(data)
                        out.absolutePath
                    }
                val row =
                    existing ?: ObxProfile(id = 0, name = p.name, type = p.type, avatarPath = avatarPath, createdAt = now, updatedAt = now)
                if (existing != null) {
                    row.name = p.name
                    row.type = p.type
                    row.avatarPath = avatarPath ?: existing.avatarPath
                    row.updatedAt = now
                }
                pbox.put(row)
            }

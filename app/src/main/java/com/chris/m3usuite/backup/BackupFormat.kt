...
/**
 * MSBX single-file backup format for m3uSuite.
 * If not encrypted: raw ZIP with:
 *   - manifest.json
 *   - payload.json
 *   - profiles/{id}/avatar.(png|jpg)
 * If encrypted: MSBX envelope (header + AES-256-GCM ciphertext of the ZIP).
 */
object BackupFormat {
    ...
    data class ProfileExport(
        ...
        val name: String,
        val type: String,
        val avatarFile: String? = null // path inside zip, e.g. "profiles/12/avatar.png"
    )
    ...
    @Serializable
    data class Payload(
        val settings: Map<String, String> = emptyMap(), // key -> JSON string (serialized primitive) for portability

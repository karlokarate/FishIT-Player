...
object BackupFormat {
    ...
    @Serializable
    data class ResumeVodMark(val mediaId: Long, val positionSecs: Int, val updatedAt: Long)

    @Serializable
    data class ResumeEpisodeMark(val episodeId: Int, val positionSecs: Int, val updatedAt: Long)
    ...
    fun zip(manifest: Manifest, payload: Payload, assets: Map<String, ByteArray> = emptyMap()): ByteArray {
    ...
    fun unzip(bytes: ByteArray): Triple<Manifest, Payload, Map<String, ByteArray>> {
        var man: Manifest? = null; var pay: Payload? = null
    ...
    private fun deriveKey(pass: CharArray, salt: ByteArray, iterations: Int = 120_000): ByteArray {
    ...
    private fun aesGcmEncrypt(zip: ByteArray, passphrase: CharArray): ByteArray {
        ...
        out.write(MAGIC.encodeToByteArray()); out.write(byteArrayOf(VERSION)); out.write(byteArrayOf(FLAG_ENCRYPTED))
        out.write(salt); out.write(iv); out.write(enc)
        return out.toByteArray()
    }

    private fun aesGcmDecrypt(envelope: ByteArray, passphrase: CharArray): ByteArray {
        require(envelope.size > 34) { "Invalid MSBX envelope" }
        val magic = envelope.copyOfRange(0, 4).decodeToString(); require(magic == MAGIC) { "Not an MSBX file" }
        val flags = envelope[5]; require(flags.toInt() and FLAG_ENCRYPTED.toInt() != 0) { "File not encrypted" }
        val salt = envelope.copyOfRange(6, 22); val iv = envelope.copyOfRange(22, 34)
    ...
    fun pack(manifest: Manifest, payload: Payload, assets: Map<String, ByteArray>, passphrase: CharArray?): ByteArray {
        val zip = zip(manifest, payload, assets)
        return if (passphrase != null) aesGcmEncrypt(zip, passphrase) else zip
    }

    fun unpack(bytes: ByteArray, passphrase: CharArray?): Triple<Manifest, Payload, Map<String, ByteArray>> {
        val isEncrypted = bytes.size > 6 && bytes.copyOfRange(0, 4).decodeToString() == MAGIC && bytes[5].toInt() and FLAG_ENCRYPTED.toInt() != 0

...
class TvKeyDebouncerTest {
    ...
    @Test
    fun `first key press is processed immediately`() = runBlocking {
        // Create a mock KeyEvent for DPAD_LEFT
        val handled = debouncer.handleKeyEventRateLimited(
    ...
    @Test
    fun `rapid key presses are debounced`() = runBlocking {
    ...
    @Test
    fun `key press after debounce period is processed`() = runBlocking {
        // First press
        debouncer.handleKeyEventRateLimited(
            keyCode = KeyEvent.KEYCODE_DPAD_LEFT,
            event = createKeyEvent(KeyEvent.KEYCODE_DPAD_LEFT, KeyEvent.ACTION_DOWN)
    ...
    @Test
    fun `different keys are debounced independently`() = runBlocking {
        // Press LEFT
        debouncer.handleKeyEventRateLimited(
            keyCode = KeyEvent.KEYCODE_DPAD_LEFT,
            event = createKeyEvent(KeyEvent.KEYCODE_DPAD_LEFT, KeyEvent.ACTION_DOWN)
    ...
    @Test
    fun `resetKey clears debouncing state for specific key`() = runBlocking {
        // First press
        debouncer.handleKeyEventRateLimited(
            keyCode = KeyEvent.KEYCODE_DPAD_LEFT,
            event = createKeyEvent(KeyEvent.KEYCODE_DPAD_LEFT, KeyEvent.ACTION_DOWN)
    ...
    @Test
    fun `ACTION_UP events are handled without processing handler`() = runBlocking {
    ...
    @Test
    fun `debouncing can be disabled`() = runBlocking {
    ...
    private fun createKeyEvent(keyCode: Int, action: Int): KeyEvent {

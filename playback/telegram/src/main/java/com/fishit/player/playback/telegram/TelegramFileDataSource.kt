 ...
 * - Uses `TelegramTransportClient` from transport layer
 ...
 *
 * @param transportClient Transport layer client for TDLib file operations
 */
class TelegramFileDataSource(
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        try {
            ...
            val fileLocation: TgFile = runBlocking {
                transportClient.requestFileDownload(
            ...
            if (!fileLocation.isDownloadingCompleted) {
                throw IOException("File not yet downloaded: fileId=$resolvedFileId, downloaded=${fileLocation.downloadedPrefixSize}/${fileLocation.size}")
            }
...
class TelegramFileDataSourceFactory(
    ...
    override fun createDataSource(): DataSource {
        return TelegramFileDataSource(transportClient)
    }
}

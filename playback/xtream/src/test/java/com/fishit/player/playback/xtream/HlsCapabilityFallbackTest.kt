...
class HlsCapabilityFallbackTest {
    ...
    fun `format selection with HLS available selects m3u8 when present`() {
        ...
        val selected = XtreamPlaybackSourceFactoryImpl.selectXtreamOutputExt(
    ...
    fun `format selection with empty formats fails regardless of HLS`() {
        ...
        val exceptionWithHls = assertThrows(PlaybackSourceException::class.java) {
            XtreamPlaybackSourceFactoryImpl.selectXtreamOutputExt(
                allowedFormats = formats,
                hlsAvailable = true
            )
        }
        val exceptionWithoutHls = assertThrows(PlaybackSourceException::class.java) {
            XtreamPlaybackSourceFactoryImpl.selectXtreamOutputExt(
                allowedFormats = formats,
                hlsAvailable = false
            )
    ...
    fun `Provider with only m3u8 requires HLS module`() {
        ...
        val selectedWithHls = XtreamPlaybackSourceFactoryImpl.selectXtreamOutputExt(
            allowedFormats = formats,
            hlsAvailable = true
        )

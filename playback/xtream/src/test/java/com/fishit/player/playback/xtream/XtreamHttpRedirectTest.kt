...
class XtreamHttpRedirectTest {
    ...
    @Test
    fun `test 302 redirect followed from server A to server B`() = runTest {
        ...
        val factory = XtreamHttpDataSourceFactory(
            context = context,
            headers = headers,
            debugMode = false
        )
    ...
    @Test
    fun `test 301 permanent redirect followed`() = runTest {
    ...
    @Test
    fun `test HTTP to HTTPS redirect allowed`() = runTest {
    ...
    @Test
    fun `test headers preserved in initial request`() = runTest {

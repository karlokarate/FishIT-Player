 ...
 */
@Singleton
class DefaultObxDatabaseInspector @Inject constructor(
    ...
    override suspend fun listRows(
        ...
        withContext(Dispatchers.IO) {
            val spec = ObxInspectorEntityRegistry.byId[entityTypeId]
            ...
            val previews = entities.mapNotNull { entity ->
    ...
    override suspend fun updateFields(
        ...
        withContext(Dispatchers.IO) {
            val spec = ObxInspectorEntityRegistry.byId[entityTypeId]
                ?: return@withContext DbUpdateResult(entityTypeId = entityTypeId, id = id, applied = 0, errors = listOf("Unknown entity type: $entityTypeId"))

            val box = boxStore.boxFor(spec.clazz)
            val entity = runCatching { box.get(id) }.getOrNull()
                ?: return@withContext DbUpdateResult(entityTypeId = entityTypeId, id = id, applied = 0, errors = listOf("Row not found (id=$id)"))

            val errors = mutableListOf<String>()
            var applied = 0

            val editableFields = entity.javaClass.declaredFields
    ...
    private sealed interface ParseResult {
        data class Ok(val value: Any?) : ParseResult
        data class Error(val message: String) : ParseResult
    }

    private fun parseStringToFieldType(field: Field, rawValue: String?): ParseResult {
        val type = field.type

...
constructor(
    ...
    override suspend fun upsertCanonicalMedia(
            ...
            withContext(Dispatchers.IO) {
                ...
                val kind =
                        if (normalized.season != null && normalized.episode != null) "episode"
                        else "movie"
                ...
                if (existing != null) {
                    ...
                    val updated =
                            existing.copy(
                                    ...
                                    tmdbId = normalized.externalIds.tmdbId ?: existing.tmdbId,
                    ...
                    CanonicalMediaId(
                            kind = if (kind == "episode") MediaKind.EPISODE else MediaKind.MOVIE,
                            key = canonicalKey
                    )
                } else {
                    // Create new entry
    ...
    override suspend fun addOrUpdateSourceRef(
            ...
            withContext(Dispatchers.IO) {
                ...
                val canonical =
                        canonicalBox
                                .query(ObxCanonicalMedia_.canonicalKey.equal(canonicalId.key))
                                .build()
                                .findFirst()
                                ?: return@withContext
                ...
                if (existing != null) {
                    ...
                    val updated =
                            existing.copy(
                                    ...
                                    qualityJson = source.quality?.let { encodeQuality(it) },
                                    languagesJson = source.languages?.let { encodeLanguages(it) },
                                    formatJson = source.format?.let { encodeFormat(it) },
            ...
            withContext(Dispatchers.IO) {
                ...
                val canonical =
                        when {
                            tmdbId != null ->
                                    canonicalBox
                                            .query(ObxCanonicalMedia_.tmdbId.equal(tmdbId))
                                            .build()
                                            .findFirst()
                            imdbId != null ->
                                    canonicalBox
                                            .query(ObxCanonicalMedia_.imdbId.equal(imdbId))
                                            .build()
                                            .findFirst()
                            tvdbId != null ->
                                    canonicalBox
                                            .query(ObxCanonicalMedia_.tvdbId.equal(tvdbId))
                                            .build()
                                            .findFirst()
                            else -> null
                        }
    ...
    override suspend fun getSourcesForMedia(canonicalId: CanonicalMediaId): List<MediaSourceRef> =
            withContext(Dispatchers.IO) {
                ...
                canonical.sources.map { toMediaSourceRef(it) }.sortedByDescending { it.priority }
            }
                        ...
                        resumeBox
                                .query(
                                        ObxCanonicalResumeMark_.profileId
                                                .equal(profileId)
                                                .and(
                                                        ObxCanonicalResumeMark_.isCompleted.equal(
                                                                false
                                                        )
                                                )
                                                .and(ObxCanonicalResumeMark_.positionMs.greater(0))
                                )
    ...
    override suspend fun getStats(): CanonicalMediaStats =
            withContext(Dispatchers.IO) {
                ...
                val movieCount =
                        canonicalBox.query(ObxCanonicalMedia_.kind.equal("movie")).build().count()

                val episodeCount =
                        canonicalBox.query(ObxCanonicalMedia_.kind.equal("episode")).build().count()

                val withTmdb =
                        canonicalBox.query(ObxCanonicalMedia_.tmdbId.notNull()).build().count()
                ...
                SourceType.entries.forEach { type ->
                    val count =
                            sourceBox
                                    .query(ObxMediaSourceRef_.sourceType.equal(type.name))
                                    .build()
                                    .count()
                    if (count > 0) {
    ...
    private fun toCanonicalResumeInfo(obx: ObxCanonicalResumeMark): CanonicalResumeInfo {
        // Use stored positionPercent if available, otherwise calculate from position/duration
        val progress =

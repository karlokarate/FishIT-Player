...
class ObxProfileRepository @Inject constructor(
    ...
    override suspend fun getAllProfiles(): List<ProfileRepository.Profile> = withContext(Dispatchers.IO) {
        profileBox.all.map { it.toDomain() }
    }
    
    override suspend fun getProfileById(id: Long): ProfileRepository.Profile? = withContext(Dispatchers.IO) {
        profileBox.get(id)?.toDomain()
    }
    
    override suspend fun getActiveProfile(): ProfileRepository.Profile = withContext(Dispatchers.IO) {
        // For Phase 2, return the first adult profile or create default
        ensureDefaultProfile()
        profileBox.query(ObxProfile_.type.equal("adult"))
            .build()
            .findFirst()
            ?.toDomain()
    ...
    override suspend fun updateProfile(id: Long, name: String, avatarPath: String?): Unit = withContext(Dispatchers.IO) {
    ...
    override suspend fun deleteProfile(id: Long): Unit = withContext(Dispatchers.IO) {
        ...
        val adultCount = profileBox.query(ObxProfile_.type.equal("adult"))
            .build()
            .count()
    ...
    override suspend fun ensureDefaultProfile(): Unit = withContext(Dispatchers.IO) {
        val count = profileBox.count()
        if (count == 0L) {
            val now = System.currentTimeMillis()
            val defaultProfile = ObxProfile(
                name = "Default",
                type = "adult",
                createdAt = now,
                updatedAt = now
    ...
    private fun ObxProfile.toDomain() = ProfileRepository.Profile(
        id = id,
        name = name,
        type = when (type) {
    ...
    private fun ProfileRepository.ProfileType.toObxType() = when (this) {

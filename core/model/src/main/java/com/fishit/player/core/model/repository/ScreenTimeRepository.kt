...
interface ScreenTimeRepository {
     ...
     */
    suspend fun getScreenTimeEntry(kidProfileId: Long, dayYyyymmdd: String): ScreenTimeEntry
    
    /**
     * Add used minutes to the screen time for a profile on a specific day.
     */
    suspend fun addUsedMinutes(kidProfileId: Long, dayYyyymmdd: String, minutes: Int)
    
    /**
     * Set the daily limit for a profile on a specific day.
     */
    suspend fun setDailyLimit(kidProfileId: Long, dayYyyymmdd: String, limitMinutes: Int)
    
    /**
     * Check if a profile has remaining screen time for a specific day.
     * @return true if usedMinutes < limitMinutes
     */
    suspend fun hasRemainingTime(kidProfileId: Long, dayYyyymmdd: String): Boolean
    
    /**
     * Get remaining minutes for a profile on a specific day.
     * @return Remaining minutes, or Int.MAX_VALUE if no limit is set
     */
    suspend fun getRemainingMinutes(kidProfileId: Long, dayYyyymmdd: String): Int

...
class TsRegressionSceneParserTest {
    ...
    private val edgeCaseTestCases =
        listOf(
            ...
            // Very long titles
            TestCase(
                "The.Lord.of.the.Rings.The.Fellowship.of.the.Ring.2001.Extended.1080p.BluRay",
                "The Lord of the Rings The Fellowship of the Ring",
                2001,
            ),
            TestCase(
                "Pirates.of.the.Caribbean.The.Curse.of.the.Black.Pearl.2003.1080p",
    ...
    private val additionalTestCases =
        listOf(
    ...
    fun `edge cases - provider prefixes removed`() {
        val providerCases =
            edgeCaseTestCases.filter {
                it.input.startsWith("N|") ||
                    it.input.startsWith("UHD|") ||
                    it.input.startsWith("D|") ||
                    it.input.contains("___NF___")
            }

        for (case in providerCases) {
            val result = parser.parse(case.input)
            assertFalse(
                result.title.startsWith("N|") ||
                    result.title.startsWith("UHD|") ||
                    result.title.startsWith("D|") ||
                    result.title.contains("___"),
    ...
    fun `all test cases produce non-empty titles`() {
        val allCases =
            movieTestCases + seriesTestCases + animeTestCases + edgeCaseTestCases +
    ...
    fun `count total test cases - should be 300 plus`() {
        val total =
            movieTestCases.size + seriesTestCases.size + animeTestCases.size +

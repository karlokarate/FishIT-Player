...
class RegexSceneNameParser : SceneNameParser {
    companion object {
        ...
        // Common provider prefixes: N|, NF|, D|, DE|, EN|, FHD|, 4K|, UHD|, HD|, SD|, VOD|, etc.
        // These appear at the start of IPTV stream titles and should be stripped

        // Provider tag prefixes (pipe-separated, bracket, or bare)
        // Pattern: strips leading sequences like "N|", "NF|", "D|DE|FHD|", "[N]", "(4K)", etc.
        private val PROVIDER_PREFIX_REGEX =
                Regex(
                        "^[\\s|_\\-*#\\[\\]()]*" + // Leading garbage
                        "(?:" +
                                "(?:N|NF|D|DE|EN|ES|IT|FR|PT|RU|PL|TR|AR|" + // Language/provider
                                // codes
                                "FHD|4K|UHD|HD\\+?|SD|HDR|" + // Quality prefixes (including HD+)
        ...
        private val DIRECTORS_REGEX =
                Regex("\\b(Director'?s?\\s*Cut|DIRECTORS?\\s*CUT)\\b", RegexOption.IGNORE_CASE)
    ...
    override fun parse(filename: String): ParsedSceneInfo {
        ...
        if (year == null) {
            val yearBracketMatch = YEAR_BRACKET_REGEX.findAll(workingString).lastOrNull()
            if (yearBracketMatch != null) {
                val yearCandidate = yearBracketMatch.groupValues[1].toIntOrNull()
                if (yearCandidate != null && yearCandidate in 1900..2099) {
                    year = yearCandidate
                    workingString = workingString.replace(yearBracketMatch.value, " ")
                }
            }
        }
        ...
        var title =
                workingString
                        ...
                        .replace(Regex("[\\[\\](){}|]"), " ") // Remove remaining brackets and pipes

...
class DefaultTmdbMetadataResolver
    ...
    constructor(
        ...
        private val movieCache =
            TmdbLruCache<Int, TmdbMovieDetail>(
                maxSize = TmdbLruCache.DEFAULT_MAX_SIZE,
                ttlMs = TmdbLruCache.TTL_7_DAYS_MS,
            )

        private val tvCache =
            TmdbLruCache<Int, TmdbShowDetail>(
                maxSize = TmdbLruCache.DEFAULT_MAX_SIZE,
                ttlMs = TmdbLruCache.TTL_7_DAYS_MS,
            )

        private val searchCache =
            TmdbLruCache<String, SearchCacheEntry>(
                maxSize = TmdbLruCache.DEFAULT_MAX_SIZE,
         ...
         */
        private fun buildTmdbPosterUrl(posterPath: String): String = "${TMDB_IMAGE_BASE_URL}w500$posterPath"
         ...
         */
        private fun buildTmdbBackdropUrl(backdropPath: String): String = "${TMDB_IMAGE_BASE_URL}w1280$backdropPath"

        private fun buildSearchCacheKey(
            normalized: NormalizedMediaMetadata,
            config: com.fishit.player.core.metadata.tmdb.TmdbConfig,
        ): String = "${normalized.canonicalTitle}|${normalized.year}|${normalized.mediaType}|${config.language}"
...
class NoOpTmdbMetadataResolver : TmdbMetadataResolver {
    override suspend fun enrich(normalized: NormalizedMediaMetadata): NormalizedMediaMetadata = normalized
}

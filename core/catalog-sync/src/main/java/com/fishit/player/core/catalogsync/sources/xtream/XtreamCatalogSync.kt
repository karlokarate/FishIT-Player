...
class XtreamCatalogSync
    ...
    constructor(
        ...
        override fun sync(config: XtreamSyncConfig): Flow<SyncStatus> =
            channelFlow {
                ...
                try {
                                        ...
                                        is XtreamCatalogEvent.ScanProgress -> {
                                            ...
                                            send(
                                                SyncStatus.InProgress(
                                                    ...
                                                    processedItems =
                                                        liveCount.get() + vodCount.get() + seriesCount.get() + episodeCount.get(),
                                                    totalItems = null,
                                                ),
                                            )
                                        }
                    ...
                    log(
                        "Sync completed: $totalItems items in ${durationMs}ms " +
                            "(live=${liveCount.get()}, vod=${vodCount.get()}, " +
                            "series=${seriesCount.get()}, episodes=${episodeCount.get()}, " +
                            "skippedTimestamp=${skippedByTimestamp.get()}, " +
                            "skippedFingerprint=${skippedByFingerprint.get()})",
                    )
                    send(
                        SyncStatus.Completed(
                            source = SYNC_SOURCE,
                            ...
                            itemCounts =
                                SyncStatus.Completed.ItemCounts(
        ...
        override suspend fun getLastSyncTime(accountKey: String): Long? =
            checkpointStore.getCheckpoint(checkpointKey(accountKey))?.lastSyncTimestamp

        // === Private Helpers ===

        private fun resolveDeviceProfile(config: XtreamSyncConfig): DeviceProfile =
            if (config.deviceProfile == DeviceProfile.AUTO) {
        ...
        private suspend fun checkSyncStrategy(config: XtreamSyncConfig): SyncStrategy =
            incrementalSyncDecider.decideSyncStrategy(
                sourceType = "xtream",
                accountId = config.accountKey,
                contentType = "all",
         ...
         */
        private fun MediaType.toContentTypeKey(): String =
            when (this) {
                MediaType.LIVE -> "live"
                MediaType.MOVIE -> "vod"

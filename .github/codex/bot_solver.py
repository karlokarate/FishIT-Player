...
def sanitize_patch(raw: str) -> str:
    ...
    for ln in txt.splitlines():
        # remove "1.:" / "2.:" or separator lines between sections
...
def _git_apply_matrix(tmp: str, uses_ab: bool) -> List[Tuple[int, bool, str]]:
    ...
    combos=[]
    for p in pseq:
        combos += [(p,True,""), (p,False,""), (p,False,"--ignore-whitespace"),
...
def whole_git_apply(patch_text: str) -> Tuple[bool, str]:
    ...
    # GNU patch (whole) – selten nötig, wir meiden globale Backups, daher aus
...
def apply_section(sec_text: str, idx: int) -> bool:
    ...
    for p,three,extra in _git_apply_matrix(tmp, uses_ab):
        pr=_git_apply_try(tmp,p,three,extra)
        if pr.returncode==0: return True
...
def summarize_rejects(num: int):
    ...
    if not rej_files: return
    preview=[]
    for rf in rej_files[:10]:
        try: txt = Path(rf).read_text(encoding="utf-8", errors="replace")
...
def main():
    ...
    # 2) Apply: whole -> section-wise (zero-context, gnu per section)
    ...
    if ok:
        ...
        if not sections:
            add_label(num, "solver-error"); post_comment(num, "❌ Solver: Keine diff --git Sections gefunden."); dispatch_triage(num); sys.exit(1)
        applied_any=False
        for i, sec in enumerate(sections, 1):
            if apply_section(sec, i): applied_any=True
        if not applied_any:
            add_label(num, "solver-error"); post_comment(num, "❌ Solver: Section-wise Apply fehlgeschlagen."); summarize_rejects(num); dispatch_triage(num); sys.exit(1)

    status = sh("git status --porcelain")
    if not status.strip():
        add_label(num, "solver-error")
        post_comment(num, "❌ Solver: Keine Änderungen im Working Tree nach Apply.")
        summarize_rejects(num); dispatch_triage(num); sys.exit(1)
...
if __name__ == "__main__":
    try: main()
        ...
        try:
            num = issue_number()
            if num: add_label(num, "solver-error"); dispatch_triage(num)
        except Exception: pass

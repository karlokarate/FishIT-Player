...
# Soft-fail & Caching; baut Host + Generator, generiert Java, baut tdjni je ABI aus example/java
#
# Wichtigste Korrekturen ggü. Vorversion:
#  1) Schemata (td_api.tl/.tlo) werden robust an den von example/java erwarteten Ort gelegt
#     (td/example/java/td/bin/td/generate/scheme), notfalls .tlo via tl-parser erzeugt.
#  2) tdjni wird NICHT im Repo-Root, sondern in example/java pro ABI mit Android-Toolchain gebaut.
#  3) java-install wird in example/java ausgeführt, nachdem Generator + Schemas vorhanden sind.
...
# (1) Host-Build & Install (liefert CMake-Package + tl-parser etc.)
...
# (1b) Generator & vorbereitete Quellen
...
# (1c) Generator & Schemata robust bereitstellen
...
if [[ -f "$SRC_TLO_HOST" ]]; then
  ...
  echo "[schema] td_api.tlo missing; try generating via tl-parser..."
  if [[ -x "build-host/td/generate/tl-parser/tl-parser" && -f "$SCHEME_DST_DIR/td_api.tl" ]]; then
    run "gen-schema-tlo" build-host/td/generate/tl-parser/tl-parser \
      -e "$SCHEME_DST_DIR/td_api.tl" \
      -o "$SCHEME_DST_DIR/td_api.tlo"
  else
    record_err "tl-parser not available or td_api.tl missing; cannot generate td_api.tlo"
  fi
fi

# PHP-Helper für Doku-Generator (optional, aber harmless)
...
# (2) example/java: Java-Quellen generieren (TdApi.java, Client.java, Log.java)
...
# (4) JNI je ABI — aus example/java mit Android-Toolchain das Target 'tdjni' bauen
...
for ABI in "${ABI_ARR[@]}"; do
  ...
  run "jni-$ABI-configure" cmake -S . -B "$BDIR" -G Ninja \
    ...
    -DTd_DIR="$TD_DIR/example/java/td/$CMAKEDIR_REL" \
    -DCMAKE_PREFIX_PATH="$TD_DIR/example/java/td" \
    -DTD_ENABLE_JNI=ON \
...
popd >/dev/null  # zurück aus $TD_DIR

# (5) Java → JAR (nur wenn Quellen vorhanden)

...
# Final: API24, Preflight-Dirs, force BoringSSL, verbose, JNI Javadoc fix, sccache/ccache auto
set -euo pipefail
...
# Tools (lokal)
ensure(){ if command -v apt-get >/dev/null 2>&1 && command -v sudo >/dev/null 2>&1; then
  for pkg in "$@"; do local apt="$pkg"; [[ "$pkg" == "ninja" ]] && apt="ninja-build"; command -v "$pkg" >/dev/null 2>&1 || { sudo apt-get update -y && sudo apt-get install -y "$apt"; }; done; fi; }
...
mkdir -p "$TD_DIR/example/java" "$HOST_INSTALL_PREFIX/$INSTALL_LIBDIR" "$HOST_INSTALL_PREFIX/$INSTALL_BINDIR" \
         "$HOST_INSTALL_PREFIX/$INSTALL_INCLUDEDIR" "$HOST_INSTALL_PREFIX/$CMAKEDIR_REL" "$DOCS_DIR"
# Host-Generator-Pfad (vorsorglich)
...
for _abi in "${ABI_ARR[@]}"; do
  ...
  mkdir -p "$ANDROID_PREFIX/$INSTALL_LIBDIR" "$ANDROID_PREFIX/$INSTALL_BINDIR" \
           "$ANDROID_PREFIX/$INSTALL_INCLUDEDIR" "$ANDROID_PREFIX/$CMAKEDIR_REL" \
           "$OUT_DIR/libs/$_abi"
  # BoringSSL je ABI vorhanden?
...
# Verhindere Host-OpenSSL via pkg-config
...
for ABI in "${ABI_ARR[@]}"; do
  ...
  Z1="$PREBUILT_DIR/sysroot/usr/lib/$TRIPLE/$API_LEVEL/libz.a"; Z2="$PREBUILT_DIR/sysroot/usr/lib/$TRIPLE/libz.a"
  ZLIB_LIB="$([[ -f "$Z1" ]] && echo "$Z1" || echo "$Z2")"
  ...
  run "core-$ABI-configure" cmake -S . -B "build-td-android-$ABI" -G Ninja \
    ...
    -DCMAKE_PREFIX_PATH="$BORINGSSL_DIR/$ABI;$ANDROID_PREFIX" \
  ...
  run "core-$ABI-build"   cmake --build "build-td-android-$ABI" "${NINJA_KEEP[@]}" || { echo ">> tail core-$ABI build log"; (find "build-td-android-$ABI" -name '*.log' -print -quit | xargs -r tail -n 200) || true; false; }
  ...
  run "jni-$ABI-configure" cmake -S . -B "$BDIR" -G Ninja \
    ...
    -DCMAKE_FIND_USE_SYSTEM_ENVIRONMENT_PATH=OFF \
    -DCMAKE_PREFIX_PATH="$ANDROID_PREFIX;$BORINGSSL_DIR/$ABI" \
  ...
  run "jni-$ABI-build-tdjni" cmake --build "$BDIR" --target tdjni "${NINJA_KEEP[@]}" || { echo ">> tail jni-$ABI build log"; (find "$BDIR" -name '*.log' -print -quit | xargs -r tail -n 200) || true; false; }

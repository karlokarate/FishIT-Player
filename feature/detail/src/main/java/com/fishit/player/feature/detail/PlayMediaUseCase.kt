 ...
 *
 * Converts [MediaSourceRef] and [CanonicalMediaId] into a [PlaybackContext]
 * and delegates to the player for playback.
 *
...
class PlayMediaUseCase @Inject constructor(
    ...
    suspend fun play(
        ...
        UnifiedLog.d(TAG) { "play.requested: canonical=${canonicalId.key}, source=${source.sourceId}" }
        ...
        try {
            ...
            val context = buildPlaybackContext(
            ...
            UnifiedLog.d(TAG) { "play.started: sourceType=${context.sourceType}, sourceKey=${context.sourceKey}" }
            ...
            SourceType.XTREAM -> {
                ...
                when {
                    sourceIdValue.startsWith("xtream:vod:") -> {
                        put("contentType", "vod")
                        put("vodId", sourceIdValue.removePrefix("xtream:vod:"))
                    }
    ...
    private fun mapToPlayerSourceType(sourceType: SourceType): com.fishit.player.core.playermodel.SourceType {
        return when (sourceType) {
            ...
            SourceType.IO, SourceType.LOCAL -> com.fishit.player.core.playermodel.SourceType.FILE
            // Unsupported sources → UNKNOWN
            SourceType.PLEX, SourceType.OTHER, SourceType.UNKNOWN -> com.fishit.player.core.playermodel.SourceType.UNKNOWN
        }
    }
     ...
     *
     * Only [ImageRef.Http] provides a directly usable URL.
     * Other variants (TelegramThumb, LocalFile, InlineBytes) require
     * resolution via Coil Fetchers and are not directly usable here.
     */
    private fun extractHttpUrl(imageRef: com.fishit.player.core.model.ImageRef): String? {
        return when (imageRef) {
            ...
            is com.fishit.player.core.model.ImageRef.TelegramThumb -> null // Requires TDLib resolution

    ...
    constructor(
        ...
        private suspend fun enrichFromXtream(media: CanonicalMediaWithSources): CanonicalMediaWithSources {
            ...
            return try {
                ...
                UnifiedLog.i(TAG) {
                    "enrichFromXtream: success canonicalId=${media.canonicalId.key.value} hasPlot=${!normalized.plot.isNullOrBlank()} hasGenres=${!normalized.genres
                        .isNullOrBlank()} containerExt=$containerExt"
                }
        ...
        private suspend fun enrichFromTmdb(media: CanonicalMediaWithSources): CanonicalMediaWithSources {
            ...
            return try {
                ...
                if (!enriched.plot.isNullOrBlank() || !enriched.genres.isNullOrBlank()) {
                    ...
                    UnifiedLog.i(TAG) {
                        "enrichFromTmdb: success canonicalId=${media.canonicalId.key.value} hasPlot=${!enriched.plot.isNullOrBlank()} hasGenres=${!enriched.genres
                            .isNullOrBlank()}"
                    }

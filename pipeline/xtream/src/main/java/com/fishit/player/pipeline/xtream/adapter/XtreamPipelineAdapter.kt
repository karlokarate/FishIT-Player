...
private fun XtreamSeriesInfo.toEpisodes(
    ...
    episodes?.forEach { (seasonNumber, episodeList) ->
        ...
        episodeList.forEach { ep ->
            // Skip episodes without a valid episodeId (stream ID).
            // Episodes without an ID cannot be played via the direct path (/series/{user}/{pass}/{episodeId}.ext)
            // and the legacy path (/series/{user}/{pass}/{seriesId}/{season}/{episode}.ext) doesn't work
            // reliably with 302 redirects on most modern Xtream panels.
            val episodeId = ep.resolvedEpisodeId
            if (episodeId == null || episodeId <= 0) {
                // Log warning but continue - this prevents breaking the entire series
                // if only some episodes are missing IDs
                return@forEach
            }

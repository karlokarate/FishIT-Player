 ...
 */
internal class SeriesItemPhase
    @Inject
    constructor(
        ...
        override suspend fun execute(
            ...
            UnifiedLog.d(
                TAG,
                "[SERIES] Starting scan (after slot available)${if (hasFilter) " with ${categoryFilter.size} category filter(s) [server-side]" else ""}...",
            )
                                    ...
                                    categorySemaphore.withPermit {
                                        source.streamSeriesItems(batchSize = config.batchSize, categoryId = catId) { batch ->
                                            for (seriesItem in batch) {
                                                ...
                                                val catalogItem = mapper.fromSeries(seriesItem, headers, config.accountLabel)

...
class TelegramBundleToMediaItemMapper
    ...
    constructor(
         ...
         */
        fun mapBundles(bundles: List<TelegramMessageBundle>): List<TelegramMediaItem> = bundles.flatMap { mapBundle(it) }

        /** Extracts structured metadata from a TEXT message. */
        private fun extractMetadata(textMessage: TgMessage): StructuredMetadata =
            metadataExtractor.extractStructuredMetadata(textMessage) ?: StructuredMetadata.EMPTY
        ...
        private fun extractVideoProperties(videoMessage: TgMessage): VideoProperties =
            when (val content = videoMessage.content) {
                is TgContent.Video ->
                    VideoProperties(

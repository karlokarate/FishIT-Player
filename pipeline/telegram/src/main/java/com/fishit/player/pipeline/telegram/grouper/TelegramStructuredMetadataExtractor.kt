 ...
 */
@Singleton
class TelegramStructuredMetadataExtractor @Inject constructor() {
    ...
    private fun applyRatingGuard(rating: Double): Double? =
            if (rating in RATING_RANGE) rating
            else {
    ...
    private fun applyLengthGuard(length: Int): Int? =
            if (length in LENGTH_RANGE) length
            else {
    ...
    private fun extractField(text: String, pattern: Regex): String? =
            pattern.find(text)?.groupValues?.getOrNull(1)

    private fun extractInt(text: String, pattern: Regex): Int? =
            extractField(text, pattern)?.toIntOrNull()

    private fun extractDouble(text: String, pattern: Regex): Double? =
            extractField(text, pattern)?.toDoubleOrNull()

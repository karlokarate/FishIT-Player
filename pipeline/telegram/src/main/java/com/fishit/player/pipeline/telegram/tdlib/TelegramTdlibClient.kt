...
/**
 * v2 TDLib Client Abstraction
 * 
 * Abstracts TDLib client operations for Telegram media access.
 * Focuses on media-related operations (chat history, message/media resolution, file locations).
 * 
...
interface TelegramTdlibClient {
     ...
     * Emits updates when auth state changes.
     */
    val authState: Flow<TelegramAuthState>
    
    /**
     * Current connection state.
     * Emits updates when connection state changes.
     */
     ...
     * 
     * If not authorized, this will initiate the auth flow.
     * Callers should observe [authState] to handle interactive auth steps.
     * 
    ...
    /**
     * Ensure a file is ready for playback (downloads if needed).
     * 
     * @param fileId TDLib file ID
     * @param priority Download priority (1-32, higher = more important)
     * @param offset Starting byte offset (0 for full file)
     * @param limit Number of bytes to download (0 = entire file)

...
class DefaultTelegramClientTest {
    ...
    @Test
    fun `ensureAuthorized succeeds when state is Ready`() =
        runTest {
    ...
    @Test
    fun `ensureAuthorized throws when waiting for phone`() =
        runTest {
    ...
    @Test
    fun `ensureAuthorized throws when waiting for code`() =
        runTest {
    ...
    @Test
    fun `ensureAuthorized throws when waiting for password`() =
        runTest {
    ...
    @Test
    fun `ensureAuthorized throws on TDLib error`() =
        runTest {
            coEvery { mockProvider.initialize() } returns Unit
            coEvery { mockTdlClient.getAuthorizationState() } returns
    ...
    @Test
    fun `ensureAuthorized initializes provider when not initialized`() =
        runTest {
    ...
    @Test
    fun `getChats returns list of chat info`() =
        runTest {
    ...
    @Test
    fun `getChats handles individual chat fetch failure gracefully`() =
        runTest {
            val chatIds = longArrayOf(1001L, 1002L)
            val chatsResult = mockk<Chats> { every { this@mockk.chatIds } returns chatIds }
    ...
    @Test
    fun `fetchMediaMessages returns media items from chat`() =
        runTest {
            val messages =
    ...
    @Test
    fun `fetchMediaMessages uses correct offset for pagination`() =
        runTest {
    ...
    @Test
    fun `fetchMediaMessages limits to max 100 per TDLib`() =
        runTest {
            val emptyMessages = mockk<Messages> { every { messages } returns emptyArray() }
    ...
    @Test
    fun `fetchAllMediaMessages aggregates from multiple chats`() =
        runTest {
    ...
    @Test
    fun `fetchAllMediaMessages handles chat failure gracefully`() =
        runTest {
            val messages1 = createMockVideoMessages(chatId = 1001L, messageIds = listOf(100L))
            val messagesResult1 =
                mockk<Messages> { every { messages } returns messages1.toTypedArray() }
    ...
    @Test
    fun `resolveFileLocation returns file details`() =
        runTest {
            val file =
                createMockFile(
                    fileId = 999,
    ...
    @Test
    fun `resolveFileByRemoteId returns file ID`() =
        runTest {
    ...
    @Test
    fun `requestFileDownload returns file location`() =
        runTest {
            val file =
                createMockFile(
                    fileId = 777,
     ...
     */
    private fun createMockVideoMessages(
        chatId: Long,
    ...
    private fun createMockFile(
        ...
        val localFile =
            TdlibTestFixtures.createLocalFile(
                path = localPath,
        ...
        val remoteFile =
            TdlibTestFixtures.createRemoteFile(
                id = remoteId,
                uniqueId = uniqueId,
            )

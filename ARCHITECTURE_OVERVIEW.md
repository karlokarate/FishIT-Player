...
- Detail: `TelegramVideoDetailScreen` konsumiert `TelegramLiveRepository.fetchMessageMediaItem(...)`, reichert die Nachricht über `DetailPage` und einen MediaAction-Play-Button an und startet Playback via `PlayUrlHelper.tgPlayUri(...)`. Start- sowie Library-Suche verwenden `searchAllVideos(query, limit)` als Mirror-Only Shortcut.

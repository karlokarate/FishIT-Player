 ...
 *
 * Executes full Telegram history synchronization via CatalogSyncService ONLY.
 * Uses parallel chat scanning with per-chat checkpointing for resumability.
 *
...
constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val existingCheckpoint = checkpointStore.getTelegramCheckpoint()
        ...
        try {
            ...
            catalogSyncService.syncTelegramPlatinum(
                            ...
                            excludeChatIds = existingCheckpoint.processedChatIds, // Resume from checkpoint
                    ...
                    .collect { status ->
                        ...
                        if (elapsedMs > input.maxRuntimeMs) {
                            ...
                            saveCheckpoint(existingCheckpoint, processedChatIds, newHighWaterMarks)
                            UnifiedLog.i(TAG) { "CHECKPOINT_SAVED processed_chats=${processedChatIds.size}" }
                            return@collect
                        }

                        when (status) {
                            is SyncStatus.Started -> {
                                UnifiedLog.d(TAG) { "PLATINUM sync started for source: ${status.source}" }
                            }
                            ...
                            is SyncStatus.Completed -> {
                                ...
                                // Final checkpoint save - use merged HWMs from existingCheckpoint + newHighWaterMarks
                                // Clear processedChatIds since sync is complete (no resume needed)
                                val finalCheckpoint = existingCheckpoint
                                    .updateHighWaterMarks(newHighWaterMarks) // Merge new HWMs
                            ...
                            is SyncStatus.SeriesEpisodeComplete -> { /* not applicable */ }
                        }
                    }
    ...
    private suspend fun saveCheckpoint(
        ...
        val checkpoint = existingCheckpoint
        ...
        UnifiedLog.d(TAG) { "Checkpoint saved: ${processedChatIds.size} chats, ${newHighWaterMarks.size} HWMs" }
    }
}

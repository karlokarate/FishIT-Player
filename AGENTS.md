...
- Media actions: Use the centralized action model and `MediaActionBar` (`ui/actions/*`) for detail‑screen CTAs (Resume/Play/Trailer/Add/Remove/OpenEPG/Share). Gate via `BuildConfig.MEDIA_ACTIONBAR_V1` (default ON). Order and Telemetry hooks (`ui_action_*`) must be consistent across screens.
 - Detail header: Use `ui/detail` (`DetailHeader`/`MetaChips`/`HeroScrim`) for VOD/Series/Live detail headers under `BuildConfig.DETAIL_SCAFFOLD_V1` (default ON). Avoid duplicating legacy header when the flag is enabled.
...
Recent
- Telegram backfill: Implemented `TelegramSyncWorker` to fetch recent messages from selected Film/Series chats and index minimal metadata into ObjectBox (`ObxTelegramMessage`). Settings now display chat names (not only IDs) for the selected sources when authenticated.

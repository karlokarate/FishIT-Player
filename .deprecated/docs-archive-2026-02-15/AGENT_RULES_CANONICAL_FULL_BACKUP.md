...
## ⚠️ CRITICAL: Agent Rules Meta-Rules (READ FIRST)
...
### ALL Agent Changes Go Through Canonical Source

> **HARD RULE:** All changes to agent rules MUST be made in `docs/meta/AGENT_RULES_CANONICAL.md` ONLY.
...
# 2. Sync to targets (AGENTS.md + .github/copilot-instructions.md)
...
**CI Protection:** The workflow `.github/workflows/verify-agent-rules.yml` fails if:
- `AGENTS.md` ≠ `.github/copilot-instructions.md`
- OR either file ≠ `docs/meta/AGENT_RULES_CANONICAL.md`

**Why This Matters:**
- Prevents drift between AGENTS.md (human-readable) and copilot-instructions.md (Copilot auto-applied)
- Single source of truth for all agent behavior
- Changes tracked in git history of canonical file only

### Inline Annotations Must Be Consolidated

> **HARD RULE:** Inline annotations, comments, and TODOs that describe implementations MUST be consolidated into contracts when they represent architectural decisions or rules.

**Examples of annotations requiring consolidation:**
```kotlin
// TODO: This violates NX_SSOT_CONTRACT INV-6 - should use NxWorkRepository
private val obxRepo: ObxCanonicalMediaRepository  // CONSOLIDATE → contract

// CRITICAL: Player MUST NOT know about Telegram sources
// (if repeated across files) → CONSOLIDATE → AGENTS.md or contract

// WORKAROUND: Using ObxWriteConfig.FIRETV_BATCH_CAP=35 for memory constraints
// → CONSOLIDATE → CATALOG_SYNC_WORKERS_CONTRACT_V2.md
```

**When to consolidate:**
- Annotation describes a cross-module rule or invariant
- Comment explains "why" of an architectural decision
- TODO references a contract violation or missing feature
- Workaround explanation applies globally

**How to consolidate:**
1. Identify if annotation belongs in contract or AGENTS.md
2. Add to appropriate document with clear reference
3. Replace inline comment with brief reference: `// See AGENTS.md Section X.Y`
4. Remove redundant multi-file duplicates

### File Header and Annotation Conflict Analysis

> **HARD RULE:** Before ANY file modification, agents MUST analyze all file headers and inline annotations to detect conflicts with planned changes.

**Required Analysis:**
```kotlin
// Example file header to analyze
/**
 * CRITICAL: This module follows NX_SSOT_CONTRACT INV-6.
 * DO NOT import Obx* entities - use NxWorkRepository only.
 * 
 * Architecture: core/catalog-sync is SSOT for all sync orchestration.
 * Workers MUST call CatalogSyncWorkScheduler, never pipelines directly.
 */
```

**Agent workflow:**
1. **Before edit:** Read entire file (not just target function)
2. **Extract:** Find all headers, inline annotations, TODOs, architecture comments
3. **Analyze:** Check if planned change conflicts with any annotation
4. **Report:** If conflict found, stop and ask user for resolution
5. **Proceed:** Only if no conflicts or user approved override

**Conflict examples:**
- Planned: Add `import com.fishit.player.infra.data.obx.*`
- Conflict: Header says "DO NOT import Obx* entities"
- Action: Stop, report conflict to user

### Roadmap Maintenance (MANDATORY)

> **HARD RULE:** Agents MUST maintain ROADMAP.md by removing completed work and adding open work.

**After completing any task:**
1. **Remove from roadmap:** Delete completed items/sections from ROADMAP.md
2. **Add new open work:** Add newly discovered tasks to appropriate theme
3. **Update status:** Change status markers (✅ → for completed, 🚧 → for in progress)
4. **Keep clean:** Only open/in-progress work should remain on roadmap

**If you notice unmaintained roadmap items:**
- Remove completed work even if not done by you
- Add missing open work discovered in TODOs or inline comments
- Restructure by themes if needed for clarity

**Why this matters:**
- Roadmap must reflect current reality, not historical artifacts
- Prevents roadmap from becoming a graveyard of completed tasks
- Makes it easy to see what's actually left to do
...
4.7. Shared HTTP Client Infrastructure (Architecture Decision)

> **ARCHITECTURE DECISION (Jan 2026):** `infra/http-client` is the SHARED HTTP infrastructure for all HTTP-based layers.

**Purpose:**
- Provides reusable HTTP client with rate limiting, caching, GZIP handling, and streaming
- Prevents code duplication across transport layers (Xtream, future M3U, Jellyfin, WebDAV)
- Contains NO business logic – purely technical HTTP infrastructure

**Who Uses It:**
- `infra/transport-xtream` – For Xtream API calls
- Future transport modules requiring HTTP (M3U, Jellyfin, WebDAV)
- `core/ui-imaging` – For image fetching via Coil (NOT a violation – imaging is separate concern)

**What It Contains:**
- OkHttp client factory with configurable timeouts
- Rate limiting interceptors
- Cache configuration (memory + disk)
- GZIP handling
- Streaming support

**Hard Rules:**
1. `infra/http-client` MUST remain business-agnostic (no Xtream/Telegram logic)
2. Transport modules MAY depend on `infra/http-client`
3. Pipelines MUST NOT depend on `infra/http-client` (use transport abstractions)
4. Feature modules MUST NOT depend on `infra/http-client` (use domain repositories)

**Reference:** See `.github/instructions/infra-transport.instructions.md` for OkHttp usage guidelines.

4.8. Global Sync Orchestration SSOT (Architecture Decision)

> **ARCHITECTURE DECISION (Jan 2026):** `core/catalog-sync` is the SINGLE SOURCE OF TRUTH for ALL sync orchestration across sources and layers.

**Purpose:**
- Central orchestrator for catalog synchronization (Telegram, Xtream, IO, future sources)
- Consumes catalog events from pipelines, normalizes via `MediaMetadataNormalizer`, persists to repositories
- Coordinates sync scheduling via `CatalogSyncWorkScheduler`

**SSOT Position:**
```
Transport → Pipeline → core/catalog-sync (SSOT) → Data → Domain → UI
```

**Hard Rules:**
1. **Workers MUST call `CatalogSyncWorkScheduler`** – NEVER direct pipeline or transport
2. **ViewModels/UI MUST call `CatalogSyncWorkScheduler`** – NEVER direct `CatalogSyncService`
3. `core/catalog-sync` NEVER touches transport directly – only via pipeline abstractions
4. All sync triggers (app-start, source activation, manual) go through scheduler

**Forbidden Patterns:**
```kotlin
// ❌ WRONG: Worker calling pipeline directly
class XtreamSyncWorker(
    private val xtreamPipeline: XtreamCatalogPipeline  // WRONG!
)

// ❌ WRONG: ViewModel calling CatalogSyncService
class SettingsViewModel(
    private val catalogSyncService: CatalogSyncService  // WRONG!
)

// ✅ CORRECT: All use scheduler
class XtreamSyncWorker(
...
**Reference:** 
- `/contracts/CATALOG_SYNC_WORKERS_CONTRACT_V2.md` (W-2, W-3: SSOT rules)
- `.github/instructions/core-catalog-sync.instructions.md`
- `.github/instructions/catalog-sync.instructions.md`

4.9. Bridge Duplicate Detection (MANDATORY)
...
0. **Instructions Read (MANDATORY - FIRST STEP)**
   ...
   - [ ] **Scope Guard Check**: If file is in `.scope/*.scope.json`, read that scope file first (see Section 17)
...
## 17. Scope Guard System (MANDATORY)

> **HARD RULE:** Before ANY file modification, agents MUST check the Scope Guard system to prevent context loss and ensure architectural compliance.

### 17.1. Problem Statement

LLM agents lose context during long conversations. Rules established at Step 1 may be "forgotten" at Step 5, causing:
- Duplicate methods inserted into files
- Invariants violated silently
- Architectural rules broken
- Cross-file dependencies ignored

### 17.2. Solution: Scope Guard Files

The `.scope/` directory contains JSON files defining bounded contexts:

```
.scope/
├── scope-guard.schema.json    # JSON Schema for validation
├── catalog-sync.scope.json    # core/catalog-sync/** (27 files, ~5400 LOC)
├── persistence.scope.json     # core/persistence/** entities + repos
└── README.md                  # Full documentation
```

Each scope file is **small enough to always fit in context** and provides:
- Complete file inventory for the scope
- Explicit invariants per critical file
- Known bugs and gaps
- Related scopes and dependencies
- Forbidden patterns

### 17.3. Mandatory Workflow

**Before editing ANY file:**

1. **Identify Scope**: Determine which `.scope/*.scope.json` covers the file:
   | File Path Pattern | Scope File |
   |-------------------|------------|
   | `core/catalog-sync/**` | `catalog-sync.scope.json` |
   | `core/persistence/**` | `persistence.scope.json` |

2. **Read Scope File**: Read the **entire** scope file. It contains:
   - `mandatoryReadBeforeEdit`: Documents to read first
   - `criticalFiles`: Files with invariants that must be maintained
   - `globalInvariants`: Rules that apply to all files in scope
   - `knownBugs`: Current bugs in specific files
   - `forbiddenPatterns`: Anti-patterns to avoid

3. **Verify No Violations**: Before making changes, confirm:
   - [ ] Planned change doesn't violate any `globalInvariants`
   - [ ] Planned change doesn't introduce any `forbiddenPatterns`
   - [ ] Planned change respects `invariants` of the specific file
   - [ ] `relatedScopes` have been considered

4. **Check for Duplicates**: The scope file lists ALL files. Before adding new code:
   - [ ] Search ALL `criticalFiles` for similar functionality
   - [ ] Check `handlers` array for existing implementations
   - [ ] Verify no duplicate exists in `diModules`

### 17.4. Scope File Structure

```json
{
  "scopeId": "catalog-sync",
  "version": "1.0.0",
  "modules": [":core:catalog-sync"],
  "totalFiles": 27,
  "estimatedLOC": 5400,
  "mandatoryReadBeforeEdit": [
    "/contracts/CATALOG_SYNC_WORKERS_CONTRACT_V2.md",
    "/docs/v2/INCREMENTAL_SYNC_CONTRACT_V1.md"
  ],
  "globalInvariants": [
    "Workers MUST call CatalogSyncWorkScheduler - NEVER direct pipeline or transport",
    "All sync triggers go through CatalogSyncWorkScheduler"
  ],
  "criticalFiles": [
    {
      "path": "core/catalog-sync/.../DefaultXtreamSyncService.kt",
      "locEstimate": 492,
      "invariants": ["Must respect SyncStrategy parameter"],
      "knownBugs": ["executePhase() ignores SyncStrategy - always does FULL"]
    }
  ],
  "forbiddenPatterns": [
    "Direct pipeline.sync() calls from workers",
    "Obx* imports in UI layer"
  ]
}
```
...
If an agent edits a file without reading its scope file first:
- This is an **architecture violation**
- The edit may need to be reverted
- Agent must explain why scope was not checked

### 17.6. Creating New Scope Files

When adding new bounded contexts:

1. Use the JSON Schema: `.scope/scope-guard.schema.json`
2. Include ALL files in the scope (complete inventory)
3. Document invariants for critical files
4. Link related scopes
5. Register in `.github/instructions/_index.instructions.md`

### 17.7. Enforcement

Scope Guards are enforced through:
- `.github/instructions/scope-guard.instructions.md` (auto-applied by Copilot)
- Pre-change checklist (Section 11.1)
- Agent mode instructions referencing this section

**All agents** (Cloud Tasks, Codespace, local VS Code) receive the scope-guard instruction automatically via the `.github/instructions/` mechanism.

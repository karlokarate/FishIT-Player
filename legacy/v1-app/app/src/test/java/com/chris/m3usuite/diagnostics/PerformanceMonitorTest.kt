...
class PerformanceMonitorTest {
    ...
    @Test
    fun `measure returns result and duration`() =
        runBlocking {
            val (result, duration) =
    ...
    @Test
    fun `measureAndLog only logs when exceeding threshold`() =
        runBlocking {
    ...
    @Test
    fun `measureAndLog logs WARN for very slow operations`() =
        runBlocking {
    ...
    @Test
    fun `measureAndLog includes additional metadata`() =
        runBlocking {
            PerformanceMonitor.measureAndLog(
                category = "test",
    ...
    @Test
    fun `Timer tracks multiple checkpoints`() =
        runBlocking {
            val timer =
    ...
    @Test
    fun `Timer with no checkpoints still works`() =
        runBlocking {
            val timer =
                PerformanceMonitor.startTimer(
                    category = "test",
    ...
    @Test
    fun `measureAndLog works with different result types`() =
        runBlocking {
            // Int result
            val intResult =
                PerformanceMonitor.measureAndLog(

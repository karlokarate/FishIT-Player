...
class InternalPlayerSessionPhase2Test {
    ...
    class FakeResumeManager : ResumeManager {
        ...
        fun isSeriesResumeCleared(
            seriesId: Int,
            season: Int,
            episodeNum: Int,
        ): Boolean = !seriesResume.containsKey("$seriesId-$season-$episodeNum")
    ...
    @Test
    fun `resumeManager does not resume when stored position is less than or equal to 10 seconds`() =
        runBlocking {
    ...
    @Test
    fun `resumeManager does not resume when stored position is exactly 10 seconds`() =
        runBlocking {
    ...
    @Test
    fun `resumeManager resumes when stored position is greater than 10 seconds`() =
        runBlocking {
    ...
    @Test
    fun `resumeManager resumes series content when stored position is greater than 10 seconds`() =
        runBlocking {
    ...
    @Test
    fun `resumeManager returns null for LIVE content`() =
        runBlocking {
    ...
    @Test
    fun `resumeManager clears resume when remaining duration is less than 10 seconds`() =
        runBlocking {
    ...
    @Test
    fun `resumeManager saves resume when remaining duration is 10 seconds or more`() =
        runBlocking {
            // Given: VOD context
            val context = PlaybackContext(type = PlaybackType.VOD, mediaId = 123L)
    ...
    @Test
    fun `resumeManager clears resume on playback ended`() =
        runBlocking {
            // Given: VOD with resume position stored
            resumeManager.setVodResume(123L, 100)
            val context = PlaybackContext(type = PlaybackType.VOD, mediaId = 123L)
    ...
    @Test
    fun `resumeManager clears series resume on playback ended`() =
        runBlocking {
    ...
    @Test
    fun `resumeManager does nothing for LIVE on ended`() =
        runBlocking {
            // Given: LIVE context
            val context = PlaybackContext(type = PlaybackType.LIVE, mediaId = 789L)
    ...
    @Test
    fun `resumeManager does not save when duration is invalid`() =
        runBlocking {
            // Given: VOD context
            val context = PlaybackContext(type = PlaybackType.VOD, mediaId = 123L)
    ...
    @Test
    fun `kidsGate returns inactive state for non-kid profile`() =
        runBlocking {
            // Given: Adult profile
    ...
    @Test
    fun `kidsGate returns active state for kid profile with remaining time`() =
        runBlocking {
    ...
    @Test
    fun `kidsGate returns blocked state for kid profile with no remaining time`() =
        runBlocking {
    ...
    @Test
    fun `kidsGate 60-second accumulation triggers quota decrement`() =
        runBlocking {
            // Given: Kid profile with time remaining
            kidsGate.isKidProfile = true
    ...
    @Test
    fun `kidsGate quota exhaustion leads to blocked state`() =
        runBlocking {
    ...
    @Test
    fun `kidsGate does not decrement for non-kid state`() =
        runBlocking {
            // Given: Non-kid state
            val currentState =
                KidsGateState(
    ...
    @Test
    fun `kidsGate handles zero or negative deltaSecs`() =
        runBlocking {
    ...
    @Test
    fun `kidsGate blocked state persists`() =
        runBlocking {
    ...
    @Test
    fun `Phase2Integration loadInitialResumePosition delegates to resumeManager`() =
        runBlocking {
    ...
    @Test
    fun `Phase2Integration onPlaybackTick handles resume and kids gate`() =
        runBlocking {
    ...
    @Test
    fun `Phase2Integration onPlaybackEnded clears resume`() =
        runBlocking {
            // Given: VOD context with resume
            resumeManager.setVodResume(123L, 100)
            val context = PlaybackContext(type = PlaybackType.VOD, mediaId = 123L)
    ...
    @Test
    fun `Phase2Integration evaluateKidsGateOnStart delegates to kidsGate`() =
        runBlocking {
            // Given: Kid profile
            kidsGate.isKidProfile = true
            kidsGate.remainingMinutes = 30

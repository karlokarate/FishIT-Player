...
class StartUseCases(
    ...
    suspend fun listByTypeFiltered(
        ...
        limit: Int,
        offset: Int,
    ): List<MediaItem> = withContext(Dispatchers.IO) { mediaRepo.listByTypeFiltered(type, limit, offset) }

    suspend fun isAllowed(
        type: String,
        id: Long,
    ): Boolean = withContext(Dispatchers.IO) { mediaRepo.isAllowed(type, id) }

    fun pagingSearchSeries(query: String): Flow<PagingData<MediaItem>> = mediaRepo.pagingSearchFilteredFlow("series", query)

    fun pagingSearchVod(query: String): Flow<PagingData<MediaItem>> = mediaRepo.pagingSearchFilteredFlow("vod", query)

    fun pagingSearchLive(query: String): Flow<PagingData<MediaItem>> = mediaRepo.pagingSearchFilteredFlow("live", query)

    fun pagingByTypeFilteredLive(provider: String?): Flow<PagingData<MediaItem>> =
        if (provider.isNullOrBlank()) {
            mediaRepo.pagingByTypeFilteredFlow("live", null)
        } else {
            mediaRepo.pagingByTypeFilteredFlow("live", provider)
        }

    suspend fun telegramSearchAllChats(
        query: String,
        limit: Int,
    ): List<MediaItem> = telegramRepo.searchTelegramContent(query).first().take(limit)

    suspend fun computeSeriesMixed(
        recent: List<MediaItem>,
        newest: List<MediaItem>,
        showAdults: Boolean,
        kid: Boolean,
    ): Pair<List<MediaItem>, Set<Long>> =

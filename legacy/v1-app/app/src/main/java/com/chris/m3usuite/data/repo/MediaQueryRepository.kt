...
class MediaQueryRepository(
    ...
    private fun obxKindPrefix(type: String): Long =
        when (type) {
    ...
    private suspend fun currentKidIdOrNull(): Long? =
    ...
    private suspend fun allowedIdsForType(
        kidId: Long,
        type: String,
    ): Set<Long> =
        withContext(Dispatchers.IO) {
    ...
    private suspend fun allowedIdsByCategories(
        kidId: Long,
        type: String,
    ): Set<Long> =
        withContext(Dispatchers.IO) {
    ...
    private suspend fun blockedIdsForType(
        kidId: Long,
        type: String,
    ): Set<Long> =
        withContext(Dispatchers.IO) {
    ...
    private suspend fun effectiveAllowedIds(
        kidId: Long,
        type: String,
    ): Set<Long> {
    ...
    suspend fun listByTypeFiltered(
        type: String,
        limit: Int,
        offset: Int,
    ): List<MediaItem> =
    ...
    suspend fun byTypeAndCategoryFiltered(
        type: String,
        cat: String?,
    ): List<MediaItem> =
        withContext(Dispatchers.IO) {
            val itemsRaw: List<MediaItem> =
                when (type) {
                    "live" -> {
                        val base =
                            if (!cat.isNullOrBlank()) {
                                ...
                                obxRepo.livePaged(0, Long.MAX_VALUE)
                            }
                    ...
                    "vod" -> {
                        val base =
                            if (!cat.isNullOrBlank()) {
                                ...
                                obxRepo.vodPaged(0, Long.MAX_VALUE)
                            }
                        base.map { it.toMediaItem(context) }
                    }
                    else -> {
                        val base =
                            if (!cat.isNullOrBlank()) {
                                ...
                                obxRepo.seriesPaged(0, Long.MAX_VALUE)
                            }
                        base.map { it.toMediaItem(context) }
                    }
                }
            val catLabelById =
                runCatching { obxRepo.categories(type).associateBy({ it.categoryId }, { it.categoryName }) }
                    .getOrElse { emptyMap() }
    ...
    suspend fun categoriesByTypeFiltered(type: String): List<String?> =
        withContext(Dispatchers.IO) {
            val showA = settings.showAdults.first()
            val cats =
                obxRepo
    ...
    private fun pagerConfig(): PagingConfig {
        val tvLow =
            com.chris.m3usuite.core.device.DeviceProfile
                .isTvLowSpec(context)
        return if (tvLow) {
            PagingConfig(
                pageSize = 16,

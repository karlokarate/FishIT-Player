...
/**
 * TDLib-Optimized Streaming Configuration for Video Playback.
 *
 ...
 *    - Typical moov size: 50KB-500KB for optimized files, can be larger
 *
 ...
 * - tdlib-coroutines best practices
...
object StreamingConfigRefactor {
    ...
    /**
     * @deprecated Old constant, use MIN_PREFIX_FOR_VALIDATION_BYTES instead.
     */
    @Deprecated(
        "Use MIN_PREFIX_FOR_VALIDATION_BYTES for clarity",
        ReplaceWith("MIN_PREFIX_FOR_VALIDATION_BYTES"),
    )
    const val DEFAULT_PREFIX_BYTES: Long = MIN_PREFIX_FOR_VALIDATION_BYTES

    /**
     * @deprecated Old constant, use MAX_PREFIX_SCAN_BYTES for header validation context.
     */
    @Deprecated(
        "Use MAX_PREFIX_SCAN_BYTES for header validation context",
        ReplaceWith("MAX_PREFIX_SCAN_BYTES"),
    )
    const val ENSURE_READY_POLL_INTERVAL_MS: Long = PREFIX_POLL_INTERVAL_MS
}

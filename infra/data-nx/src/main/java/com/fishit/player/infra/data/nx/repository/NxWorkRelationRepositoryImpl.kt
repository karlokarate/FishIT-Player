 ...
 */
@Singleton
class NxWorkRelationRepositoryImpl
    @Inject
    constructor(
        ...
        override suspend fun upsert(relation: Relation): Relation =
            withContext(Dispatchers.IO) {
                val parentWork =
                    workBox
                        .query(NX_Work_.workKey.equal(relation.parentWorkKey, StringOrder.CASE_SENSITIVE))
                        .build()
                        .findFirst() ?: error("Parent work not found: ${relation.parentWorkKey}")

                val childWork =
                    workBox
                        .query(NX_Work_.workKey.equal(relation.childWorkKey, StringOrder.CASE_SENSITIVE))
                        .build()
                ...
                try {
                    ...
                    boxStore.runInTx {
                        ...
                        val existingRelations =
                            box
                                .query(
                                    NX_WorkRelation_.parentWorkKey
                                        .oneOf(batchParentKeys, StringOrder.CASE_SENSITIVE)
        ...
        override suspend fun delete(
            ...
            withContext(Dispatchers.IO) {
                val childWork =
                    workBox
                        .query(NX_Work_.workKey.equal(childWorkKey, StringOrder.CASE_SENSITIVE))
                        .build()
                        .findFirst() ?: return@withContext false

                val existing =
                    findExistingRelation(parentWorkKey, childWork.id, relationType.name)
        ...
        private fun findExistingRelation(
            ...
            box
                .query(
                    NX_WorkRelation_.parentWorkKey
                        .equal(parentWorkKey, StringOrder.CASE_SENSITIVE)

 ...
 */
@Singleton
class NxCanonicalMediaRepositoryImpl
    @Inject
    constructor(
        boxStore: BoxStore,
        ...
        override suspend fun findByCanonicalId(canonicalId: CanonicalMediaId): CanonicalMediaWithSources? =
            withContext(Dispatchers.IO) {
                val work =
                    workBox
                        .query(NX_Work_.workKey.equal(canonicalId.key.value))
                        .build()
            ...
            withContext(Dispatchers.IO) {
                val work =
                    when {
                        tmdbId != null ->
                            workBox
                                .query(NX_Work_.tmdbId.equal(tmdbId.value.toString()))
                                .build()
                                .findFirst()
                        imdbId != null ->
                            workBox
                                .query(NX_Work_.imdbId.equal(imdbId))
                                .build()
                                .findFirst()
                        tvdbId != null ->
                            workBox
                                .query(NX_Work_.tvdbId.equal(tvdbId))
                                .build()
                                .findFirst()
                        else -> null
        ...
        override suspend fun findBySourceId(sourceId: PipelineItemId): CanonicalMediaWithSources? =
            withContext(Dispatchers.IO) {
                val sourceRef =
                    sourceRefBox
                        .query(
        ...
        override suspend fun getSourcesForMedia(canonicalId: CanonicalMediaId): List<MediaSourceRef> =
            withContext(Dispatchers.IO) {
                ...
                getSourceRefsForWork(work.workKey)
                    .map { mapToMediaSourceRef(it) }
        ...
        override suspend fun getResumeList(
            ...
            withContext(Dispatchers.IO) {
                ...
                userStates.mapNotNull { userState ->
                    val work =
                        workBox
                            .query(NX_Work_.workKey.equal(userState.workKey))
                            .build()
        ...
        private fun mapToMediaSourceRef(sourceRef: NX_WorkSourceRef): MediaSourceRef {
            ...
            val sourceLabel =
                SourceLabelBuilder.buildLabelWithQuality(
            ...
            val format =
                bestVariant?.containerFormat?.let { container ->
                    MediaFormat(
                        container = container,
        ...
        private fun workTypeToKind(workType: String): MediaKind = MediaTypeMapper.workTypeStringToMediaKind(workType)

        /** Delegates to [MediaTypeMapper.fromMediaKind] — SSOT. NX_CONSOLIDATION_PLAN Phase 7 #2. */
        private fun kindToWorkType(kind: MediaKind): String = MediaTypeMapper.fromMediaKind(kind)
    }

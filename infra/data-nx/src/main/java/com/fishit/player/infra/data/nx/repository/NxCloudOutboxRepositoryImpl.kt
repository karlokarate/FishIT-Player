 ...
 */
@Singleton
class NxCloudOutboxRepositoryImpl @Inject constructor(
    boxStore: BoxStore,
    ...
    override suspend fun enqueue(event: OutboxEvent): OutboxEvent = withContext(Dispatchers.IO) {
        val entity = event.toEntity()
        box.put(entity)
        entity.toDomain()
    }

    override suspend fun dequeueBatch(nowMs: Long, limit: Int): List<OutboxEvent> = withContext(Dispatchers.IO) {
        box.query()
    ...
    override suspend fun markSuccess(eventId: String): Boolean = withContext(Dispatchers.IO) {
    ...
    override suspend fun markFailure(eventId: String, nowMs: Long, error: String): Boolean = withContext(Dispatchers.IO) {
        val id = eventId.toLongOrNull() ?: return@withContext false
        val entity = box.get(id) ?: return@withContext false

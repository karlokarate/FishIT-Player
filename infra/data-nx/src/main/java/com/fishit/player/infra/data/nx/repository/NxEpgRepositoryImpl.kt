 ...
 */
@Singleton
class NxEpgRepositoryImpl @Inject constructor(
    boxStore: BoxStore,
) : NxEpgRepository {
    ...
    override suspend fun getNowNext(channelWorkKey: String): NowNext = withContext(Dispatchers.IO) {
        val nowMs = System.currentTimeMillis()

        // Find current program (startMs <= now < endMs)
        val now = box.query(NX_EpgEntry_.channelWorkKey.equal(channelWorkKey, StringOrder.CASE_SENSITIVE))
            .and()
        ...
        val next = box.query(NX_EpgEntry_.channelWorkKey.equal(channelWorkKey, StringOrder.CASE_SENSITIVE))
            .and()
    ...
    override suspend fun getSchedule(
        ...
        box.query(NX_EpgEntry_.channelWorkKey.equal(channelWorkKey, StringOrder.CASE_SENSITIVE))
            .and()
    ...
    override suspend fun searchByTitle(
        ...
        box.query(NX_EpgEntry_.titleLower.contains(queryLower, StringOrder.CASE_INSENSITIVE))
            .and()
    ...
    override suspend fun upsert(entry: EpgEntry): EpgEntry = withContext(Dispatchers.IO) {
        val nowMs = System.currentTimeMillis()
        val epgEntryKey = generateEpgEntryKey(entry.channelWorkKey, entry.startMs)

        // Find existing
        val existing = box.query(NX_EpgEntry_.epgEntryKey.equal(epgEntryKey, StringOrder.CASE_SENSITIVE))
            .build()
            .findFirst()

        val entity = NX_EpgEntry(
            id = existing?.id ?: 0,
    ...
    override suspend fun upsertBatch(entries: List<EpgEntry>): Int = withContext(Dispatchers.IO) {
    ...
    override suspend fun deleteOlderThan(beforeMs: Long): Int = withContext(Dispatchers.IO) {
        val toDelete = box.query(NX_EpgEntry_.endMs.less(beforeMs))
            .build()
            .find()
    ...
    override suspend fun deleteForChannel(channelWorkKey: String): Int = withContext(Dispatchers.IO) {
        val toDelete = box.query(NX_EpgEntry_.channelWorkKey.equal(channelWorkKey, StringOrder.CASE_SENSITIVE))
            .build()
            .find()
    ...
    override suspend fun countForChannel(channelWorkKey: String): Long = withContext(Dispatchers.IO) {
        box.query(NX_EpgEntry_.channelWorkKey.equal(channelWorkKey, StringOrder.CASE_SENSITIVE))
            .build()
            .count()
    }

    override suspend fun pruneExpired(maxAgeMs: Long): Int = withContext(Dispatchers.IO) {
    ...
    private fun generateEpgEntryKey(channelWorkKey: String, startMs: Long): String =
        "$channelWorkKey:$startMs"

    private fun NX_EpgEntry.toDomain(): EpgEntry = EpgEntry(
        epgEntryKey = epgEntryKey,

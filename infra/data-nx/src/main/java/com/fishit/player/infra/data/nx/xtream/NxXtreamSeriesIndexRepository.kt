...
@Singleton
class NxXtreamSeriesIndexRepository
    @Inject
    constructor(
        ...
        override fun observeSeasons(seriesId: Int): Flow<List<SeasonIndexItem>> {
            ...
            return flowOf(Unit).map {
                val seriesWorkKey =
                    findSeriesWorkKeyBySeriesId(seriesId)
        ...
        override suspend fun getEpisodeCount(
            ...
            withContext(Dispatchers.IO) {
                ...
                relationRepository
                    .findEpisodesForSeries(seriesWorkKey)
        ...
        override suspend fun getEpisodeBySourceKey(sourceKey: String): EpisodeIndexItem? =
            withContext(Dispatchers.IO) {
                ...
                val sourceRef =
                    sourceRefRepository.getBySourceKey(sourceKey)
                        ?: return@withContext null

                // Get the episode work
                val episodeWork =
                    workRepository.get(sourceRef.workKey)
                        ?: return@withContext null
                ...
                val episodeInfo =
                    SourceKeyParser.extractXtreamEpisodeInfo(sourceKey)
                        ?: return@withContext null
                ...
                val playbackHintsJson =
                    defaultVariant?.let { variant ->
                ...
                EpisodeIndexItem(
                    ...
                    sourceKey = sourceKey,
                    episodeId =
                        defaultVariant?.playbackHints?.get(PlaybackHintKeys.Xtream.EPISODE_ID)?.toIntOrNull()
        ...
        override suspend fun getPlaybackHints(sourceKey: String): EpisodePlaybackHints? =
            withContext(Dispatchers.IO) {
                ...
                val streamId =
                    variant.playbackHints[PlaybackHintKeys.Xtream.EPISODE_ID]?.toIntOrNull()
                        ?: episodeId
        ...
        private suspend fun findSeriesWorkKeyBySeriesId(seriesId: Int): String? {
            val sourceRefs =
                sourceRefRepository.findBySourceTypeAndKind(
         ...
         */
        private suspend fun buildSeasonIndex(
            seriesId: Int,

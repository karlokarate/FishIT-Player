 ...
 */
@Singleton
class NxWorkUserStateRepositoryImpl @Inject constructor(
    ...
    override suspend fun get(
        profileKey: String,
        workKey: String,
    ): WorkUserState? = withContext(Dispatchers.IO) {
        box.query {
        ...
        }.findFirst()?.let { mapper.toDomain(it) }
    }
    ...
    override suspend fun removeFromFavorites(
        ...
        val profileId = profileKey.toLongOrNull() ?: 0L
        val existing = findByCompositeKey(profileId, workKey) ?: run {
    ...
    private fun findByCompositeKey(profileId: Long, workKey: String): NX_WorkUserState? =
        box.query {

 ...
 */
@Singleton
class NxWorkAuthorityRepositoryImpl
    @Inject
    constructor(
        boxStore: BoxStore,
        ...
        override suspend fun findWorkKeyByAuthorityKey(authorityKey: String): String? =
            withContext(Dispatchers.IO) {
                workBox
                    .query(NX_Work_.authorityKey.equal(authorityKey, StringOrder.CASE_SENSITIVE))
                    .build()
                    .findFirst()
                    ?.workKey
            }
        ...
        override suspend fun upsert(ref: AuthorityRef): AuthorityRef =
            withContext(Dispatchers.IO) {
                val work =
                    workBox
                        .query(NX_Work_.workKey.equal(ref.workKey, StringOrder.CASE_SENSITIVE))
                        .build()
        ...
        override suspend fun delete(authorityKey: String): Boolean =
            withContext(Dispatchers.IO) {
                val work =
                    workBox
                        .query(NX_Work_.authorityKey.equal(authorityKey, StringOrder.CASE_SENSITIVE))

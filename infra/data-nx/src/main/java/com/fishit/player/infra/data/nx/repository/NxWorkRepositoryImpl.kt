...
class NxWorkRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun get(workKey: String): Work? =
        ...
        override suspend fun getBatch(workKeys: List<String>): Map<String, Work> =
            withContext(Dispatchers.IO) {
        ...
        override suspend fun searchByTitle(
            ...
            withContext(Dispatchers.IO) {
                box
                    .query(NX_Work_.canonicalTitleLower.contains(queryNormalized.lowercase(), StringOrder.CASE_INSENSITIVE))
                    .order(NX_Work_.canonicalTitle)
                    .build()
                    .find(0, limit.toLong())
                    .map { it.toDomain() }
        ...
        override fun pagingSourceFactory(options: NxWorkRepository.QueryOptions): () -> androidx.paging.PagingSource<Int, Work> =
            {
        ...
        override suspend fun count(options: NxWorkRepository.QueryOptions): Int =
            withContext(Dispatchers.IO) {
        ...
        override suspend fun getYearRange(type: WorkType?): Pair<Int, Int>? =
            withContext(Dispatchers.IO) {
        ...
        private fun applySorting(
            ...
            val flags =
                if (sortDirection == NxWorkRepository.SortDirection.DESCENDING) {
        ...
        override suspend fun upsert(work: Work): Work =
            withContext(Dispatchers.IO) {
                // Primary lookup by workKey
                var existing =
                    box
                        .query(NX_Work_.workKey.equal(work.workKey, StringOrder.CASE_SENSITIVE))
                        .build()
                        .findFirst()
                ...
                if (existing == null && work.tmdbId != null) {
                    existing =
                        box
                            .query(NX_Work_.tmdbId.equal(work.tmdbId!!))
                            .build()
                            .findFirst()
                }
                ...
                val effectiveWork =
                    if (existing != null && existing.workKey != work.workKey) {
         ...
         */
        override suspend fun enrichIfAbsent(
            workKey: String,
         ...
         */
        override suspend fun enrichFromDetail(
            workKey: String,
            enrichment: NxWorkRepository.Enrichment,
        ): NxWorkRepository.Work? =
            withContext(Dispatchers.IO) {
         ...
         */
        private fun <T> contentGuard(
            overwrite: Boolean,
            existing: T?,
            newValue: T?,
        ): T? = if (overwrite) MappingUtils.alwaysUpdate(existing, newValue) else MappingUtils.enrichOnly(existing, newValue)

        override suspend fun upsertBatch(works: List<Work>): List<Work> =
            withContext(Dispatchers.IO) {
        ...
        override suspend fun softDelete(workKey: String): Boolean =
            withContext(Dispatchers.IO) {

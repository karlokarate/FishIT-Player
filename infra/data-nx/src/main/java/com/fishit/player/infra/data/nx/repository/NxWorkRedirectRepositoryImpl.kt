 ...
 */
@Singleton
class NxWorkRedirectRepositoryImpl @Inject constructor(
    boxStore: BoxStore,
     ...
     */
    override suspend fun resolve(workKey: String): String = withContext(Dispatchers.IO) {
        var currentKey = workKey
        var depth = 0
        val maxDepth = 10 // Prevent infinite loops

        while (depth < maxDepth) {
            val redirect = box.query(NX_WorkRedirect_.oldWorkKey.equal(currentKey, StringOrder.CASE_SENSITIVE))
                .build()
                .findFirst()
    ...
    override suspend fun upsertRedirect(redirect: Redirect): Redirect = withContext(Dispatchers.IO) {
        val existing = box.query(NX_WorkRedirect_.oldWorkKey.equal(redirect.obsoleteWorkKey, StringOrder.CASE_SENSITIVE))
            .build()
            .findFirst()
    ...
    override suspend fun delete(obsoleteWorkKey: String): Boolean = withContext(Dispatchers.IO) {
        val deleted = box.query(NX_WorkRedirect_.oldWorkKey.equal(obsoleteWorkKey, StringOrder.CASE_SENSITIVE))
            .build()
            .remove()
        deleted > 0
    }

 ...
 */
@Singleton
class NxSourceAccountRepositoryImpl
    @Inject
    constructor(
        boxStore: BoxStore,
        ...
        override suspend fun get(accountKey: String): SourceAccount? =
            withContext(Dispatchers.IO) {
                box
                    .query(NX_SourceAccount_.accountKey.equal(accountKey, StringOrder.CASE_SENSITIVE))
                    .build()
                    .findFirst()
                    ?.toDomain()
            }
        ...
        override suspend fun upsert(account: SourceAccount): SourceAccount =
            withContext(Dispatchers.IO) {
                val existing =
                    box
                        .query(NX_SourceAccount_.accountKey.equal(account.accountKey, StringOrder.CASE_SENSITIVE))
                        .build()
                        .findFirst()
        ...
        override suspend fun delete(accountKey: String): Boolean =
            withContext(Dispatchers.IO) {
                val deleted =
                    box
                        .query(NX_SourceAccount_.accountKey.equal(accountKey, StringOrder.CASE_SENSITIVE))

 ...
 */
@Singleton
class NxCategorySelectionRepositoryImpl
    @Inject
    constructor(
        ...
        override fun observeForAccount(accountKey: String): Flow<List<CategorySelection>> =
            box
                .query(NX_XtreamCategorySelection_.accountKey.equal(accountKey, StringOrder.CASE_SENSITIVE))
        ...
        override suspend fun isSelected(
            ...
            withContext(Dispatchers.IO) {
                ...
                box
                    .query(NX_XtreamCategorySelection_.selectionKey.equal(key, StringOrder.CASE_SENSITIVE))
                    .build()
                    .findFirst()
                    ?.isSelected ?: false
            }
        ...
        override suspend fun upsert(selection: CategorySelection): CategorySelection =
            withContext(Dispatchers.IO) {
                val existing =
                    box
                        .query(
        ...
        override suspend fun upsertAll(selections: List<CategorySelection>) =
            withContext(Dispatchers.IO) {
        ...
        override suspend fun deleteForAccount(accountKey: String): Int =
            withContext(Dispatchers.IO) {
         ...
         */
        override suspend fun setCategorySelectionComplete(
            accountKey: String,
            complete: Boolean,
        ) {
            val key = booleanPreferencesKey("xtream:$accountKey:categorySelectionComplete")

 ...
 */
@Singleton
class NxWorkRuntimeStateRepositoryImpl
    @Inject
    constructor(
        boxStore: BoxStore,
        ...
        override suspend fun get(workKey: String): RuntimeState? =
            withContext(Dispatchers.IO) {
                box
                    .query(NX_WorkRuntimeState_.workKey.equal(workKey, StringOrder.CASE_SENSITIVE))
                    .build()
                    .findFirst()
                    ?.toDomain()
            }
        ...
        override fun observeForWorks(workKeys: List<String>): Flow<List<RuntimeState>> =
            box
                .query(NX_WorkRuntimeState_.workKey.oneOf(workKeys.toTypedArray(), StringOrder.CASE_SENSITIVE))
                .build()
                .asFlow()
        ...
        override suspend fun upsert(state: RuntimeState): RuntimeState =
            withContext(Dispatchers.IO) {
                val existing =
                    box
                        .query(NX_WorkRuntimeState_.workKey.equal(state.workKey, StringOrder.CASE_SENSITIVE))
                        .build()
                        .findFirst()
        ...
        override suspend fun upsertBatch(states: List<RuntimeState>): List<RuntimeState> =
            withContext(Dispatchers.IO) {
                states
                    .map { state ->
                        val existing =
        ...
        override suspend fun delete(workKey: String): Boolean =
            withContext(Dispatchers.IO) {
                val deleted =
                    box
                        .query(NX_WorkRuntimeState_.workKey.equal(workKey, StringOrder.CASE_SENSITIVE))

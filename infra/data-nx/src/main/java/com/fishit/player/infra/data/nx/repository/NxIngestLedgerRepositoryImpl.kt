 ...
 */
@Singleton
class NxIngestLedgerRepositoryImpl
    @Inject
    constructor(
        boxStore: BoxStore,
        ...
        override suspend fun get(ledgerKey: String): LedgerEntry? =
            withContext(Dispatchers.IO) {
                box
                    .query(NX_IngestLedger_.sourceKey.equal(ledgerKey, StringOrder.CASE_SENSITIVE))
                    .build()
                    .findFirst()
                    ?.toDomain()
            }

        override suspend fun shouldSkip(
            ledgerKey: String,
            nowMs: Long,
        ): Boolean =
            withContext(Dispatchers.IO) {
        ...
        override suspend fun upsert(entry: LedgerEntry): LedgerEntry =
            withContext(Dispatchers.IO) {
                val existing =
                    box
                        .query(NX_IngestLedger_.sourceKey.equal(entry.ledgerKey, StringOrder.CASE_SENSITIVE))
                        .build()
                        .findFirst()
        ...
        override suspend fun upsertBatch(entries: List<LedgerEntry>): List<LedgerEntry> =
            withContext(Dispatchers.IO) {
                val existingMap =
                    entries
                        .mapNotNull { entry ->

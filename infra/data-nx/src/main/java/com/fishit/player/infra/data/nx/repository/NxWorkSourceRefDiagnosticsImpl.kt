 ...
 */
@Singleton
class NxWorkSourceRefDiagnosticsImpl
    @Inject
    constructor(
        ...
        override suspend fun findMissingAccountKey(limit: Int): List<SourceRef> =
            withContext(Dispatchers.IO) {
                box
                    .query(NX_WorkSourceRef_.accountKey.equal("", StringOrder.CASE_SENSITIVE))
                    .build()
                    .find(0, limit.toLong())
                    .map { it.toDomain() }
            }

        override suspend fun findInvalidSourceKeyFormat(limit: Int): List<SourceRef> =
            withContext(Dispatchers.IO) {
        ...
        override suspend fun findOrphaned(limit: Int): List<SourceRef> =
            withContext(Dispatchers.IO) {

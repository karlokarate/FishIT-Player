 ...
 */
@Singleton
class NxProfileRuleRepositoryImpl @Inject constructor(
    boxStore: BoxStore,
    ...
    override suspend fun upsert(rule: ProfileRule): ProfileRule = withContext(Dispatchers.IO) {
        val profile = profileBox.query(NX_Profile_.profileKey.equal(rule.profileKey, StringOrder.CASE_SENSITIVE))
            .build()
        ...
        val existing = box.query(NX_ProfileRule_.profileId.equal(profile.id))
            .and()
    ...
    override suspend fun delete(
        ...
        val deleted = box.query(NX_ProfileRule_.profileId.equal(profile.id))
            .and()
            .equal(NX_ProfileRule_.ruleType, ruleTypeKey, StringOrder.CASE_SENSITIVE)
            .and()

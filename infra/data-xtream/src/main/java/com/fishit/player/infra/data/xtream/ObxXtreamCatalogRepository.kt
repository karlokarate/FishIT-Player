...
class ObxXtreamCatalogRepository
    ...
    constructor(
        ...
        override fun observeVod(categoryId: String?): Flow<List<RawMediaMetadata>> {
            ...
            // No eager loading needed. See ObxEagerPlans for pattern reference if relations are added.
            val query =
                if (categoryId != null) {
            ...
            withContext(Dispatchers.IO) {
                ...
                when (parts[1]) {
                    "vod" -> {
                        ...
                        vodBox
                            .query(ObxVod_.vodId.equal(vodId.toLong()))
                            .build()
                            .findFirst()
                            ?.toRawMediaMetadata()
                    }
        ...
        override suspend fun search(
            ...
            withContext(Dispatchers.IO) {
                ...
                val vods =
                    vodBox
                        .query(ObxVod_.nameLower.contains(lowerQuery))
                        .build()
                        .find(0, limit.toLong())
                        .map { it.toRawMediaMetadata() }
        ...
        private fun upsertVods(vods: List<ObxVod>) {
            ...
            val existingEntities =
                vodBox
                    .query(ObxVod_.vodId.oneOf(vodIds))
                    .build()
                    .find()
            ...
            UnifiedLog.d(
                TAG,
                "upsertVods: ${vods.size} items (${existingEntities.size} updates, ${vods.size - existingEntities.size} inserts)",
            )
        }
        ...
        private fun upsertSeriesEntities(series: List<ObxSeries>) {
            ...
            val existingEntities =
                seriesBox
                    .query(ObxSeries_.seriesId.oneOf(seriesIds))
                    .build()
                    .find()
            ...
            UnifiedLog.d(
                TAG,
                "upsertSeriesEntities: ${series.size} items (${existingEntities.size} updates, ${series.size - existingEntities.size} inserts)",
            )
        }
        ...
        private fun upsertEpisodes(episodes: List<ObxEpisode>) {
            ...
            val existingEpisodes =
                episodeBox
                    .query(ObxEpisode_.seriesId.oneOf(seriesIds))
                    .build()
                    .find()

            // Build lookup map with composite key "seriesId:season:episodeNum"
            val existingIdMap =
                existingEpisodes.associateBy(

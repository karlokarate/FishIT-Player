...
class ObxXtreamCatalogRepository @Inject constructor(
    ...
    override fun observeEpisodes(seriesId: String, seasonNumber: Int?): Flow<List<RawMediaMetadata>> {
        ...
        val query = if (seasonNumber != null) {
            episodeBox.query(
                ObxEpisode_.seriesId.equal(seriesIdInt.toLong())
                    .and(ObxEpisode_.season.equal(seasonNumber.toLong()))
            ).order(ObxEpisode_.episodeNum).build()
        } else {
            episodeBox.query(ObxEpisode_.seriesId.equal(seriesIdInt.toLong()))
    ...
    override suspend fun getAll(mediaType: MediaType?, limit: Int, offset: Int): List<RawMediaMetadata> =
        withContext(Dispatchers.IO) {
            when (mediaType) {
                MediaType.MOVIE -> vodBox.query().order(ObxVod_.nameLower).build()
                    .find(offset.toLong(), limit.toLong()).map { it.toRawMediaMetadata() }
                MediaType.SERIES -> seriesBox.query().order(ObxSeries_.nameLower).build()
                    .find(offset.toLong(), limit.toLong()).map { it.toRawMediaMetadata() }
                MediaType.SERIES_EPISODE -> episodeBox.query().order(ObxEpisode_.seriesId).build()
                    .find(offset.toLong(), limit.toLong()).map { it.toRawMediaMetadata() }
                else -> {
        ...
        withContext(Dispatchers.IO) {
            ...
            when (parts[1]) {
                ...
                "series" -> {
                    ...
                    seriesBox.query(ObxSeries_.seriesId.equal(seriesId.toLong()))
                        .build()
                        .findFirst()
                        ?.toRawMediaMetadata()
                }
                "episode" -> {
                    ...
                    episodeBox.query(
                        ObxEpisode_.seriesId.equal(seriesId.toLong())
                            .and(ObxEpisode_.season.equal(season.toLong()))
                            .and(ObxEpisode_.episodeNum.equal(episodeNum.toLong()))
                    ).build().findFirst()?.toRawMediaMetadata()
                }
                else -> null
            }
        }
    ...
    private fun upsertVods(vods: List<ObxVod>) {
        val toUpsert = vods.map { vod ->
            val existing = vodBox.query(ObxVod_.vodId.equal(vod.vodId.toLong()))
                .build()
                .findFirst()
    ...
    private fun upsertSeriesEntities(series: List<ObxSeries>) {
        val toUpsert = series.map { s ->
            val existing = seriesBox.query(ObxSeries_.seriesId.equal(s.seriesId.toLong()))
                .build()
                .findFirst()
    ...
    private fun upsertEpisodes(episodes: List<ObxEpisode>) {
        val toUpsert = episodes.map { ep ->
            val existing = episodeBox.query(
                ObxEpisode_.seriesId.equal(ep.seriesId.toLong())
                    .and(ObxEpisode_.season.equal(ep.season.toLong()))
                    .and(ObxEpisode_.episodeNum.equal(ep.episodeNum.toLong()))
            ).build().findFirst()
    ...
    private fun ObxVod.toRawMediaMetadata(): RawMediaMetadata = RawMediaMetadata(
        originalTitle = name,
    ...
    private fun RawMediaMetadata.toObxVod(): ObxVod? {
        val vodId = sourceId.removePrefix("xtream:vod:").toIntOrNull() ?: return null
        return ObxVod(
            vodId = vodId,
            name = originalTitle,

...
class DefaultXtreamApiClient
    ...
    constructor(
        ...
        ): Result<XtreamCapabilities> = connectionManager.initialize(source, forceDiscovery)

        override fun close() = connectionManager.close()

        override suspend fun ping(): Boolean = connectionManager.ping()

        override suspend fun getServerInfo(): Result<XtreamServerInfo> = connectionManager.getServerInfo()

        override suspend fun getPanelInfo(): String? = connectionManager.getPanelInfo()

        override suspend fun getUserInfo(): Result<XtreamUserInfo> = connectionManager.getUserInfo()

        // =========================================================================
        // Category Methods (Delegated to CategoryFetcher)
        // =========================================================================

        override suspend fun getLiveCategories(): List<XtreamCategory> = categoryFetcher.getLiveCategories()
        ...
        override suspend fun getSeriesCategories(): List<XtreamCategory> = categoryFetcher.getSeriesCategories()
        ...
        override suspend fun getLiveStreams(
            ...
            limit: Int,
            offset: Int,
        ): List<XtreamLiveStream> = streamFetcher.getLiveStreams(categoryId, limit, offset)

        override suspend fun getVodStreams(
            categoryId: String?,
            limit: Int,
            offset: Int,
        ): List<XtreamVodStream> = streamFetcher.getVodStreams(categoryId, limit, offset)

        override suspend fun getSeries(
            categoryId: String?,
            limit: Int,
            offset: Int,
        ): List<XtreamSeriesStream> = streamFetcher.getSeries(categoryId, limit, offset)
        ...
        override suspend fun countVodStreams(categoryId: String?): Int = streamFetcher.countVodStreams(categoryId)

        override suspend fun countSeries(categoryId: String?): Int = streamFetcher.countSeries(categoryId)

        override suspend fun countLiveStreams(categoryId: String?): Int = streamFetcher.countLiveStreams(categoryId)
        ...
        override suspend fun getVodInfo(
            vodId: Int,
            vodKind: String?,
        ): XtreamVodInfo? = streamFetcher.getVodInfo(vodId, vodKind)

        override suspend fun getSeriesInfo(seriesId: Int): XtreamSeriesInfo? = streamFetcher.getSeriesInfo(seriesId)
        ...
        override suspend fun getShortEpg(
            streamId: Int,
            limit: Int,
        ): List<XtreamEpgProgramme> = streamFetcher.getShortEpg(streamId, limit)

        override suspend fun getFullEpg(streamId: Int): List<XtreamEpgProgramme> = streamFetcher.getFullEpg(streamId)
        ...
        override fun buildLiveUrl(
            ...
            extension: String?,
            liveKind: String?,
        ): String = connectionManager.buildLiveUrl(streamId, extension, liveKind)

        override fun buildVodUrl(
            vodId: Int,
            containerExtension: String?,
            vodKind: String?,
        ): String = connectionManager.buildVodUrl(vodId, containerExtension, vodKind)
        ...
        override fun buildSeriesEpisodeUrl(
            ...
            connectionManager.buildSeriesEpisodeUrl(
                seriesId,
                seasonNumber,
                episodeNumber,
                episodeId,
        ...
        override fun buildCatchupUrl(
            ...
            start: Long,
            duration: Int,
        ): String? = connectionManager.buildCatchupUrl(streamId, start, duration)
        ...
        override suspend fun search(
            ...
            limit: Int,
        ): XtreamSearchResults = streamFetcher.search(query, types, limit)

        override suspend fun rawApiCall(
            action: String,
            params: Map<String, String>,
        ): String? = connectionManager.rawApiCall(action, params)
    }

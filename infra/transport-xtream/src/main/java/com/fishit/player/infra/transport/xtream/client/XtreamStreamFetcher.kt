        ...
        private suspend fun <T> streamFromUrl(
            ...
            return fetchRawAsStream(url).use { streamResp ->
                if (!streamResp.isSuccessful || streamResp.inputStream == null) {
                    UnifiedLog.w(TAG) {
                        "streamFromUrl: Request failed for ${redactUrl(
                            url,
                        )} | isSuccessful=${streamResp.isSuccessful} hasInputStream=${streamResp.inputStream != null}"
                    }
                    return@use 0
                }
                ...
                if (stats.totalCount == 0) {
                    UnifiedLog.w(TAG) {
                        "streamFromUrl: ZERO items parsed from ${redactUrl(
                            url,
                        )} | batchCount=${stats.batchCount} errors=${stats.errorCount}"
                    }
                }

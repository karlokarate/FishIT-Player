...
class DefaultTelegramTransportClient(
    ...
    override suspend fun close() {
        ...
        _authState.value = TdlibAuthState.Idle
    ...
    private fun mapAuthorizationState(state: AuthorizationState): TdlibAuthState =
            when (state) {
                is AuthorizationStateReady -> TdlibAuthState.Ready
                is AuthorizationStateWaitPhoneNumber -> TdlibAuthState.WaitPhoneNumber()
                is AuthorizationStateWaitCode -> TdlibAuthState.WaitCode()
                is AuthorizationStateWaitPassword -> TdlibAuthState.WaitPassword()
                is AuthorizationStateClosing,
                is AuthorizationStateClosed,
                is AuthorizationStateLoggingOut -> TdlibAuthState.Idle
                else -> TdlibAuthState.Connecting
            }
    ...
    private fun mapChatType(type: ChatType?): TgChatType =
            when (type) {
                ...
                is ChatTypeSupergroup ->
                        if (type.isChannel) TgChatType.CHANNEL else TgChatType.SUPERGROUP
            ...
            when (content) {
                ...
                is MessageAudio ->
                        TgContent.Audio(
                                ...
                                caption = content.caption.text.takeIf { it.isNotEmpty() },
                                albumCoverThumbnail =
                                        content.audio.albumCoverThumbnail?.let { mapThumbnail(it) },
                                albumCoverMinithumbnail =
                                        mapMinithumbnail(content.audio.albumCoverMinithumbnail)
                        )
                is MessagePhoto ->
                        TgContent.Photo(
                                sizes =

 ...
 */
@Singleton
class ObxTelegramContentRepository
    @Inject
    constructor(
        ...
        override suspend fun search(
            ...
            withContext(Dispatchers.IO) {
                ...
                box
                    .query(
                        ObxTelegramMessage_.captionLower
                            .contains(lowerQuery)
        ...
        override suspend fun upsertAll(items: List<RawMediaMetadata>) =
            withContext(Dispatchers.IO) {
                ...
                val existingEntities =
                    box
                        .query(ObxTelegramMessage_.chatId.oneOf(chatIds))
                        .build()
                        .find()

                // Build lookup map with composite key "chatId:messageId"
                val existingIdMap =
                    existingEntities.associateBy(
        ...
        private fun RawMediaMetadata.toObxEntity(): ObxTelegramMessage? {
            ...
            val thumbRemoteId =
                (thumbnail as? ImageRef.TelegramThumb)?.remoteId

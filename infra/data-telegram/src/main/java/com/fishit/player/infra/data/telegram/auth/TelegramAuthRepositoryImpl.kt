...
class TelegramAuthRepositoryImpl internal constructor(
    ...
    private fun TransportAuthState.toDomain(): DomainAuthState =
        when (this) {
            TransportAuthState.Ready -> DomainAuthState.Connected
            TransportAuthState.Connecting -> DomainAuthState.Idle
            TransportAuthState.Idle -> DomainAuthState.Idle
            TransportAuthState.LoggingOut, TransportAuthState.Closed, TransportAuthState.LoggedOut ->
                DomainAuthState.Disconnected
            is TransportAuthState.WaitPhoneNumber -> DomainAuthState.WaitingForPhone
            is TransportAuthState.WaitCode -> DomainAuthState.WaitingForCode
            is TransportAuthState.WaitPassword -> DomainAuthState.WaitingForPassword
            is TransportAuthState.Error -> DomainAuthState.Error(this.message)
            is TransportAuthState.Unknown -> DomainAuthState.Error("Unknown auth state: ${this.raw}")
        }
}
